...
// vector<vector<int>> maze(int row, vector<int>(int low)); //定义一个矩阵，用于存放数据
...
void loadToMap(vector<vector<int>> array)
{
    ...
    Mat img = Mat(h, w, CV_8UC1); //保存为RGB，图像列数像素要除以3；
    for (int i = 0; i < h; i++)
    {
        for (int j = 0; j < w; j++)
        {
...
int main()
{
    ...
    cout << "开始处理图片" << endl;
    string filePath = "/home/next/ros_workspace/map_file/AStart/mapload1.jpg";
    ...
    //     {
    //         maze[i][j] = (map.at<uchar>(i, j) == 255) ? 0 : 1; //img的矩阵数据传给二维数组maze[][]
    //         cout << maze[i][j];
    //     }
    //     cout << endl;
    // }
