...
bool isCanreach(const Point *point, const Point *target, bool isIgnoreCorner, vector<vector<int>> Array)
{
    ...
    if (target->x < 0 || target->x > Array.size() - 1 || target->y < 0 || target->y > Array[0].size() - 1 || Array[target->x][target->y] == 0 || Array[target->x][target->y] == 150 || target->x == point->x && target->y == point->y || isInList(closeList, target))
    {
...
cv::Mat imageToVector(string filePath)
{
    using namespace cv;
    ...
    // int width1 = mapNew.cols;
    // int height1 = mapNew.rows;
...
int main()
{
    ...
    cv::Mat map = imageToVector(filePath);

    cout << "A*算法地图初始化和写入" << endl;

    int width = map.cols;
    int height = map.rows;
    vector<vector<int>> maze(height, vector<int>(width, 255));     //定义一个矩阵，用于存放数据
    vector<vector<int>> mazeshow(height, vector<int>(width, 255)); //定义一个矩阵，用于方便查看膨胀过后路径的效果
