...
double Bezier2Poly::getScale() const { return scale_; }
...
void Bezier2Poly::calculatePolynomial() {
  if (degree_ == 3) {
    ...
    for (size_t i = 0; i < tValues_.size(); ++i) {
      polyX.push_back(B0_[i] * P0.x + B1_[i] * P1.x + B2_[i] * P2.x
                      + B3_[i] * P3.x);
      polyY.push_back(B0_[i] * P0.y + B1_[i] * P1.y + B2_[i] * P2.y
                      + B3_[i] * P3.y);
    }
...
std::vector<double> Bezier2Poly::linspace(double start, double end,
                                          size_t numPoints) {
...
std::vector<double> Bezier2Poly::polynomialFit(const std::vector<double> &x,
  ...
  for (size_t i = 0; i < x.size(); ++i) {
    for (int j = 0; j <= order; ++j) { A(i, j) = std::pow(x[i], j); }
    B(i) = y[i];
  }

  Eigen::VectorXd coeffs = A.colPivHouseholderQr().solve(B);
  std::vector<double> coefficients(order + 1);
  for (int i = 0; i <= order; ++i) { coefficients[i] = coeffs(i); }
  //  std::reverse(coefficients.begin(), coefficients.end());
  return coefficients;
}

std::function<double(double)>
Bezier2Poly::polynomialFunction(const std::vector<double> &coeffs) {
