...
// {
//  double t_1 = 1.0 - t;
//  return pow(t_1, 3) * ctrlPoly[0] + 3 * pow(t_1, 2) * t * ctrlPoly[1] + 3 *
//  t_1 * pow(t, 2) * ctrlPoly[2] + pow(t, 3) * ctrlPoly[3];
//}
//
// Point PiecewiseBezierFit2::qprime(const std::array<Point, 4> &ctrlPoly,
// double t) {
//  double t_1 = 1.0 - t;
//  return 3 * pow(t_1, 2) * (ctrlPoly[1] - ctrlPoly[0]) + 6 * t_1 * t *
//  (ctrlPoly[2] - ctrlPoly[1]) + 3 * pow(t, 2) * (ctrlPoly[3] - ctrlPoly[2]);
//}
//
// Point PiecewiseBezierFit2::qprimeprime(const std::array<Point, 4> &ctrlPoly,
// double t) {
//  return 6 * (1.0 - t) * (ctrlPoly[2] - 2 * ctrlPoly[1] + ctrlPoly[0]) + 6 *
//  (t) * (ctrlPoly[3] - 2 * ctrlPoly[2] + ctrlPoly[1]);
//}
...
// 二阶贝塞尔曲线参数方程
Eigen::Vector2d
PiecewiseBezierFit2::cal_bezier_value(double t, const Eigen::Vector2d &p0,
...
std::vector<double>
PiecewiseBezierFit2::chordLengthParameterize(const std::vector<Point> &points) {
  std::vector<double> u;
  u.push_back(0.0);
...
std::vector<std::vector<Point>>
PiecewiseBezierFit2::fitCubicBezier(std::vector<Point> &points,
...
std::vector<double>
PiecewiseBezierFit2::reparameterize(const std::vector<Point> &bezier,
...
*/
  ...
  //    double denominator = d_curve.x * d_curve.x + d_curve.y * d_curve.y +
  //    err.x * dd_curve.x + err.y * dd_curve.y;
...
std::pair<double, int>
PiecewiseBezierFit2::computeMaxError(const std::vector<Point> &points,
...
// 贝塞尔曲线生成函数
std::vector<Point>
PiecewiseBezierFit2::bezier_curve(const std::vector<Point> &control_points,
                                  int num_points) {
...
void PiecewiseBezierFit2::composeTrimmingSdf() {
  ...
  for (const auto &con_point : control_points_) {
    std::unique_ptr<Bezier2Poly> bezier2poly =
        std::make_unique<Bezier2Poly>(con_point, degree_);
