...
/** \brief
    ...
    with zero curvature at the start and either positive (p) or negative (n)
    max. curvature at the goal configuration.
    It evaluates all Dubins families and returns the shortest path.
    */
class CC0pm_Dubins_State_Space : public HC_CC_State_Space
{
public:
  /** \brief Constructor */
