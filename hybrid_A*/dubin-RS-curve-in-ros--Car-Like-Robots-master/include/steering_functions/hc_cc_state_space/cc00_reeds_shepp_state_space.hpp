...
class CC00_Reeds_Shepp_State_Space : public HC_CC_State_Space {
 public:
  /** \brief Constructor */
  CC00_Reeds_Shepp_State_Space(double kappa, double sigma,
                               double discretization = 0.1);

  /** \brief Destructor */
  ~CC00_Reeds_Shepp_State_Space();

  /** \brief Returns a sequence of turns and straight lines connecting the two
   * circles c1 and c2 */
  HC_CC_RS_Path* cc00_circles_rs_path(HC_CC_Circle const& c1,
                                      HC_CC_Circle const& c2) const;

  /** \brief Returns a sequence of turns and straight lines connecting a start
   * and an end configuration */
  HC_CC_RS_Path* cc00_reeds_shepp(State const& state1,
  ...
  std::vector<Control> get_controls(State const& state1,
                                    State const& state2) const override;
