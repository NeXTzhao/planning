...
void HC_CC_Circle_Param::set_param(double _kappa, double _sigma, double _radius,
                                   double _mu, double _sin_mu, double _cos_mu,
                                   double _delta_min) {
  kappa = _kappa;
  kappa_inv = 1 / _kappa;
  sigma = _sigma;
...
HC_CC_Circle::HC_CC_Circle(Configuration const& _start, bool _left,
                           bool _forward, bool _regular,
                           HC_CC_Circle_Param const& _param) {
  start = _start;
  left = _left;
  forward = _forward;
  regular = _regular;
  ...
  if (left) {
    ...
    sigma = _param.sigma;
    if (forward)
      global_frame_change(_start.x, _start.y, _start.theta, delta_x, delta_y,
                          &xc, &yc);
    else
      global_frame_change(_start.x, _start.y, _start.theta, -delta_x, delta_y,
                          &xc, &yc);
...
HC_CC_Circle::HC_CC_Circle(double _xc, double _yc, bool _left, bool _forward,
                           bool _regular, HC_CC_Circle_Param const& _param) {
...
double HC_CC_Circle::deflection(Configuration const& q) const {
...
double HC_CC_Circle::rs_circular_deflection(double delta) const {
  // regular rs-turn
  if (this->regular) return delta;
  // irregular rs-turn
  else
    return (delta <= PI) ? delta : delta - TWO_PI;
}

double HC_CC_Circle::rs_turn_length(Configuration const& q) const {
  assert(fabs(fabs(this->kappa) - fabs(q.kappa)) < get_epsilon() &&
         fabs(fabs(this->sigma) - numeric_limits<double>::max()) <
             get_epsilon());
...
double HC_CC_Circle::hc_turn_length(Configuration const& q) const {
  assert(fabs(fabs(this->kappa) - fabs(q.kappa)) < get_epsilon());
  double delta = this->deflection(q);
  return fabs(this->kappa / this->sigma) +
         fabs(this->kappa_inv * this->hc_circular_deflection(delta));
}

bool HC_CC_Circle::cc_elementary_sharpness(Configuration const& q, double delta,
                                           double& sigma0) const {
  double distance = point_distance(this->start.x, this->start.y, q.x, q.y);
  //   existence conditions for an elementary path (also see: A. Scheuer and T.
  //   Fraichard, "Continuous-Curvature Path Planning for Car-Like Vehicles," in
...
double HC_CC_Circle::cc_turn_length(Configuration const& q) const {
...
double center_distance(HC_CC_Circle const& c1, HC_CC_Circle const& c2) {
  return sqrt(pow(c2.xc - c1.xc, 2) + pow(c2.yc - c1.yc, 2));
}

bool configuration_on_hc_cc_circle(HC_CC_Circle const& c,
                                   Configuration const& q) {
