...
Configuration::Configuration(double _x, double _y, double _theta,
                             double _kappa) {
  x = _x;
  y = _y;
...
double configuration_distance(Configuration const& q1,
                              Configuration const& q2) {
  return point_distance(q1.x, q1.y, q2.x, q2.y);
}

bool configuration_aligned(Configuration const& q1, Configuration const& q2) {
...
bool configuration_equal(Configuration const& q1, Configuration const& q2) {
  if (fabs(q2.theta - q1.theta) > get_epsilon()) return false;
  if (configuration_distance(q1, q2) > get_epsilon()) return false;
  return true;
}
