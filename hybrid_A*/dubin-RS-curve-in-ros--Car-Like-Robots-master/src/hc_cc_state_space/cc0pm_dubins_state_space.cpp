...
#include "cc0pm_dubins_state_space.hpp"
...
class CC0pm_Dubins_State_Space::CC0pm_Dubins {
 ...
 public:
  explicit CC0pm_Dubins(CC0pm_Dubins_State_Space* parent) { parent_ = parent; }
  ...
  double TT_path(HC_CC_Circle const& c1, HC_CC_Circle const& c2,
    ...
    *cstart = new HC_CC_Circle(c1.start, c1.left, c1.forward, true,
                               parent_->hc_cc_circle_param_);
    *cend = new HC_CC_Circle(**q1, c2.left, !c2.forward, true,
                             parent_->hc_cc_circle_param_);
  ...
  double TiST_path(HC_CC_Circle const& c1, HC_CC_Circle const& c2,
    ...
    *cstart = new HC_CC_Circle(c1.start, c1.left, c1.forward, true,
                               parent_->hc_cc_circle_param_);
    *cend = new HC_CC_Circle(**q2, c2.left, !c2.forward, true,
                             parent_->hc_cc_circle_param_);
  ...
  double TTT_path(HC_CC_Circle const& c1, HC_CC_Circle const& c2,
    ...
    middle1 = new HC_CC_Circle(*qa, !c1.left, c1.forward, true,
                               parent_->hc_cc_circle_param_);
    end1 = new HC_CC_Circle(*qb, c2.left, !c2.forward, true,
                            parent_->hc_cc_circle_param_);
    middle2 = new HC_CC_Circle(*qc, !c1.left, c1.forward, true,
                               parent_->hc_cc_circle_param_);
    end2 = new HC_CC_Circle(*qd, c2.left, !c2.forward, true,
                            parent_->hc_cc_circle_param_);
    ...
    *q3 = new Configuration(c2.start.x, c2.start.y, c2.start.theta, c2.kappa);
    ...
    double length1 = (*cstart)->cc_turn_length(*qa) +
                     middle1->cc_turn_length(*qb) + end1->hc_turn_length(**q3);
    double length2 = (*cstart)->cc_turn_length(*qc) +
                     middle2->cc_turn_length(*qd) + end2->hc_turn_length(**q3);
    if (length1 < length2) {
      *cend = end1;
      *q1 = qa;
      *q2 = qb;
...
CC0pm_Dubins_State_Space::CC0pm_Dubins_State_Space(double kappa, double sigma,
...
CC_Dubins_Path* CC0pm_Dubins_State_Space::cc0pm_circles_dubins_path(
  ...
  if (cc0pm_dubins_->distance < get_epsilon()) {
    cstart[cc_dubins::T] = new HC_CC_Circle(c1.start, c1.left, c1.forward, true,
                                            hc_cc_circle_param_);
  ...
  if (cc0pm_dubins_->TT_exists(c1, c2)) {
    length[cc_dubins::TT] = cc0pm_dubins_->TT_path(
        c1, c2, &cstart[cc_dubins::TT], &cend[cc_dubins::TT],
        &qi1[cc_dubins::TT], &qi2[cc_dubins::TT]);
  }
  // ##### Dubins families:
  // ##################################################### case TST
  if (cc0pm_dubins_->TST_exists(c1, c2)) {
    length[cc_dubins::TST] = cc0pm_dubins_->TST_path(
        c1, c2, &cstart[cc_dubins::TST], &cend[cc_dubins::TST],
        &qi1[cc_dubins::TST], &qi2[cc_dubins::TST], &qi3[cc_dubins::TST]);
  }
  // case TTT
  if (cc0pm_dubins_->TTT_exists(c1, c2)) {
    length[cc_dubins::TTT] = cc0pm_dubins_->TTT_path(
        c1, c2, &cstart[cc_dubins::TTT], &cend[cc_dubins::TTT],
        &qi1[cc_dubins::TTT], &qi2[cc_dubins::TTT], &qi3[cc_dubins::TTT],
        &ci1[cc_dubins::TTT]);
  }
label_end:
  // select shortest path
  cc_dubins::path_type best_path =
      (cc_dubins::path_type)array_index_min(length, nb_cc_dubins_paths);
  CC_Dubins_Path* path;
  path = new CC_Dubins_Path(c1.start, c2.start, best_path, kappa_, sigma_,
                            qi1[best_path], qi2[best_path], qi3[best_path],
                            nullptr, cstart[best_path], cend[best_path],
                            ci1[best_path], nullptr, length[best_path]);
...
CC_Dubins_Path* CC0pm_Dubins_State_Space::cc0pm_dubins(
  ...
  if (forwards_) {
    ...
    end_circle[1] =
        new HC_CC_Circle(end2, false, false, true, rs_circle_param_);
  } else {
    start_circle[0] =
  ...
  // compute the shortest path for the 4 combinations (2 circles at the
  // beginning and 2 at the end)
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      ...
      if (j == 0 && state2.kappa < 0)
        ...
        continue;
      path[2 * i + j] =
          cc0pm_circles_dubins_path(*start_circle[i], *end_circle[j]);
      if (path[2 * i + j]) {
...
double CC0pm_Dubins_State_Space::get_distance(State const& state1,
...
vector<Control> CC0pm_Dubins_State_Space::get_controls(
    State const& state1, State const& state2) const {
