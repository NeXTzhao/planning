...
#include "hc_cc_state_space/paths.hpp"
...
Path::Path(Configuration const& _start, Configuration const& _end,
...
CC_Dubins_Path::CC_Dubins_Path(Configuration const& _start,
                               Configuration const& _end,
...
HC_CC_RS_Path::HC_CC_RS_Path(Configuration const& _start,
                             Configuration const& _end,
...
void cc_default_controls(HC_CC_Circle const& c, Configuration const& q,
                         double delta, bool order, vector<Control>& controls) {
  double length_min = fabs(c.kappa / c.sigma);
  ...
  // 0 < delta < 2 * delta_min
    ...
    if (cc_elementary_controls(c, q, delta, order, controls_elementary)) {
      ...
      double length_elementary =
          accumulate(controls_elementary.begin(), controls_elementary.end(),
                     0.0, [](double sum, Control const& control) {
      ...
      (length_elementary < length_default)
          ? controls.insert(controls.end(), controls_elementary.begin(),
                            controls_elementary.end())
          : controls.insert(controls.end(), controls_default.begin(),
                            controls_default.end());
      return;
    } else {
