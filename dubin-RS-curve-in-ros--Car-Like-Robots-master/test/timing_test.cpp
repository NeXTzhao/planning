...
double get_mean(vector<double> const& v) {
  double sum = accumulate(v.begin(), v.end(), 0.0);
  return sum / v.size();
}

double get_std(vector<double> const& v) {
...
int get_curv_discont(vector<Control> const& controls) {
  int curv_discont = 0;
  vector<Control>::const_iterator iter1 = controls.begin();
  for (vector<Control>::const_iterator iter2 = next(controls.begin());
       iter2 != controls.end(); ++iter2) {
    if (fabs(iter1->kappa + iter1->sigma * fabs(iter1->delta_s) -
             iter2->kappa) > EPS_KAPPA)
      ++curv_discont;
    iter1 = iter2;
  }
  return curv_discont;
...
void write_to_file(string const& id, vector<Statistic> const& stats) {
  string path_to_output = ros::package::getPath("steering_functions") +
                          "/test/" + id + "_stats.csv";
  ...
  for (auto const& stat : stats) {
    ...
    f << start.x << " " << start.y << " " << start.theta << " " << start.kappa
      << " " << start.d << ",";
    f << goal.x << " " << goal.y << " " << goal.theta << " " << goal.kappa
      << " " << goal.d << ",";
    f << stat.computation_time << "," << stat.path_length << ","
...
vector<Statistic> get_controls(string const& id, vector<State> const& starts,
                               vector<State> const& goals) {
  clock_t clock_start;
  ...
  double path_length;
  int curv_discont;
  for (auto start = starts.begin(), goal = goals.begin(); start != starts.end();
       ++start, ++goal) {
    if (id == "CC_Dubins") {
      clock_start = clock();
      ...
      clock_start = clock();
      vector<Control> hc0pm_rs_controls =
      ...
      clock_start = clock();
      vector<Control> hcpm0_rs_controls =
      ...
      clock_start = clock();
      vector<Control> hcpmpm_rs_controls =
...
vector<Statistic> get_path(string const& id, vector<State> const& starts,
                           vector<State> const& goals) {
  clock_t clock_start;
  clock_t clock_finish;
  Statistic stat;
...
vector<Statistic> get_path_with_covariance(
    string const& id, vector<State_With_Covariance> const& starts,
    vector<State> const& goals) {
  clock_t clock_start;
  clock_t clock_finish;
  Statistic stat;
  ...
  for (start = starts.begin(), goal = goals.begin(); start != starts.end();
       ++start, ++goal) {
    if (id == "CC_Dubins") {
      clock_start = clock();
...
TEST(Timing, getControls) {
  ...
  for (auto const& stat : cc_dubins_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + cc_dubins_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string cc00_dubins_id = "CC00_Dubins";
  vector<Statistic> cc00_dubins_stats = get_controls(
      cc00_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.

      clear();

  for (auto const& stat : cc00_dubins_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + cc00_dubins_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string cc0pm_dubins_id = "CC0pm_Dubins";
  vector<Statistic> cc0pm_dubins_stats = get_controls(
      cc0pm_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.

      clear();

  for (auto const& stat : cc0pm_dubins_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + cc0pm_dubins_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string ccpm0_dubins_id = "CCpm0_Dubins";
  vector<Statistic> ccpm0_dubins_stats = get_controls(
      ccpm0_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.

      clear();

  for (auto const& stat : ccpm0_dubins_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + ccpm0_dubins_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string ccpmpm_dubins_id = "CCpmpm_Dubins";
  vector<Statistic> ccpmpm_dubins_stats = get_controls(
      ccpmpm_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.

      clear();

  for (auto const& stat : ccpmpm_dubins_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + ccpmpm_dubins_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string dubins_id = "Dubins";
  vector<Statistic> dubins_stats = get_controls(
      dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.

      clear();

  for (auto const& stat : dubins_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + dubins_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string cc00_rs_id = "CC00_RS";
  vector<Statistic> cc00_rs_stats = get_controls(
      cc00_rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.

      clear();

  for (auto const& stat : cc00_rs_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + cc00_rs_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string hc_rs_id = "HC_RS";
  vector<Statistic> hc_rs_stats =
      get_controls(hc_rs_id, starts_with_curvature, goals_with_curvature);
  computation_times.

      clear();

  for (auto const& stat : hc_rs_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + hc_rs_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string hc00_rs_id = "HC00_RS";
  vector<Statistic> hc00_rs_stats = get_controls(
      hc00_rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.

      clear();

  for (auto const& stat : hc00_rs_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + hc00_rs_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string hc0pm_rs_id = "HC0pm_RS";
  vector<Statistic> hc0pm_rs_stats = get_controls(
      hc0pm_rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.

      clear();

  for (auto const& stat : hc0pm_rs_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + hc0pm_rs_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string hcpm0_rs_id = "HCpm0_RS";
  vector<Statistic> hcpm0_rs_stats = get_controls(
      hcpm0_rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.

      clear();

  for (auto const& stat : hcpm0_rs_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + hcpm0_rs_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string hcpmpm_rs_id = "HCpmpm_RS";
  vector<Statistic> hcpmpm_rs_stats = get_controls(
      hcpmpm_rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.

      clear();

  for (auto const& stat : hcpmpm_rs_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + hcpmpm_rs_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;

  string rs_id = "RS";
  vector<Statistic> rs_stats =
      get_controls(rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.

      clear();

  for (auto const& stat : rs_stats) {
    computation_times.push_back(stat.computation_time);
  }
  cout << "[----------] " + rs_id + " mean [s] +/- std [s]: "
       << get_mean(computation_times) << " +/- " << get_std(computation_times)
       << endl;
...
TEST(Timing, getPath) {
  ...
  string cc00_dubins_id = "CC00_Dubins";
  vector<Statistic> cc00_dubins_stats = get_path(
      cc00_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string cc0pm_dubins_id = "CC0pm_Dubins";
  vector<Statistic> cc0pm_dubins_stats = get_path(
      cc0pm_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string ccpm0_dubins_id = "CCpm0_Dubins";
  vector<Statistic> ccpm0_dubins_stats = get_path(
      ccpm0_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string ccpmpm_dubins_id = "CCpmpm_Dubins";
  vector<Statistic> ccpmpm_dubins_stats = get_path(
      ccpmpm_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.
...
TEST(Timing, getPathWithCovariance) {
  ...
  controller.k2 = K2;
  controller.k3 = K3;
  cc_dubins_forwards_ss.set_filter_parameters(motion_noise, measurement_noise,
                                              controller);
  cc00_dubins_forwards_ss.set_filter_parameters(motion_noise, measurement_noise,
                                                controller);
  cc0pm_dubins_forwards_ss.set_filter_parameters(motion_noise,
                                                 measurement_noise, controller);
  ccpm0_dubins_forwards_ss.set_filter_parameters(motion_noise,
                                                 measurement_noise, controller);
  ccpmpm_dubins_forwards_ss.set_filter_parameters(
      motion_noise, measurement_noise, controller);
  dubins_forwards_ss.set_filter_parameters(motion_noise, measurement_noise,
                                           controller);
  dubins_backwards_ss.set_filter_parameters(motion_noise, measurement_noise,
                                            controller);
  ...
  hc0pm_rs_ss.set_filter_parameters(motion_noise, measurement_noise,
                                    controller);
  hcpm0_rs_ss.set_filter_parameters(motion_noise, measurement_noise,
                                    controller);
  hcpmpm_rs_ss.set_filter_parameters(motion_noise, measurement_noise,
                                     controller);
  ...
  string cc_dubins_id = "CC_Dubins";
  vector<Statistic> cc_dubins_stats = get_path_with_covariance(
      cc_dubins_id, starts_with_curvature, goals_with_curvature);
  computation_times.
  ...
  string cc00_dubins_id = "CC00_Dubins";
  vector<Statistic> cc00_dubins_stats = get_path_with_covariance(
      cc00_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string cc0pm_dubins_id = "CC0pm_Dubins";
  vector<Statistic> cc0pm_dubins_stats = get_path_with_covariance(
      cc0pm_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string ccpm0_dubins_id = "CCpm0_Dubins";
  vector<Statistic> ccpm0_dubins_stats = get_path_with_covariance(
      ccpm0_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string ccpmpm_dubins_id = "CCpmpm_Dubins";
  vector<Statistic> ccpmpm_dubins_stats = get_path_with_covariance(
      ccpmpm_dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string dubins_id = "Dubins";
  vector<Statistic> dubins_stats = get_path_with_covariance(
      dubins_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string cc00_rs_id = "CC00_RS";
  vector<Statistic> cc00_rs_stats = get_path_with_covariance(
      cc00_rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string hc_rs_id = "HC_RS";
  vector<Statistic> hc_rs_stats = get_path_with_covariance(
      hc_rs_id, starts_with_curvature, goals_with_curvature);
  computation_times.
  ...
  string hc00_rs_id = "HC00_RS";
  vector<Statistic> hc00_rs_stats = get_path_with_covariance(
      hc00_rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string hc0pm_rs_id = "HC0pm_RS";
  vector<Statistic> hc0pm_rs_stats = get_path_with_covariance(
      hc0pm_rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string hcpm0_rs_id = "HCpm0_RS";
  vector<Statistic> hcpm0_rs_stats = get_path_with_covariance(
      hcpm0_rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string hcpmpm_rs_id = "HCpmpm_RS";
  vector<Statistic> hcpmpm_rs_stats = get_path_with_covariance(
      hcpmpm_rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.
  ...
  string rs_id = "RS";
  vector<Statistic> rs_stats = get_path_with_covariance(
      rs_id, starts_without_curvature, goals_without_curvature);
  computation_times.
