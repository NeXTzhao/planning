...
class Test_HC_CC_State_Space : public HC_CC_State_Space {
 public:
  ...
  vector<Control> get_controls(State const& state1, State const& state2) const {
    throw runtime_error(
        "Test_HC_CC_State_Space.get_controls() not implemented");
  }

  HC_CC_Circle_Param get_hc_cc_circle_param() { return hc_cc_circle_param_; }
};

Configuration get_cc_goal_config(double delta,
                                 HC_CC_Circle_Param const& hc_cc_circle_param) {
  double radius = hc_cc_circle_param.radius;
  double mu = hc_cc_circle_param.mu;
  double xc = radius * sin(mu);
  double yc = radius * cos(mu);

  // goal config of cc-turn given in local frame (aligned with start config)
  // assuming left = forward = true
  Configuration goal;
...
Configuration get_hc_goal_config(double delta,
  ...
  // goal config of hc-turn given in local frame (aligned with start config)
  // assuming left = forward = true
  Configuration goal;
...
vector<double> get_linear_samples(double start, double stop, size_t num) {
  ...
  double sample = start;
  for (vector<double>::iterator iter = samples.begin(); iter != samples.end();
       ++iter, sample += step)
    *iter = sample;
  return samples;
}
...
TEST(HC_CC_Circle, pathLength) {
  ...
  for (auto const& sigma : sigmas) {
    ...
    HC_CC_Circle reg_hc_cc_circle =
        HC_CC_Circle(start_config, true, true, true, hc_cc_circle_param);
    HC_CC_Circle irreg_hc_cc_circle =
        HC_CC_Circle(start_config, true, true, false, hc_cc_circle_param);

    for (auto const& delta : deltas) {
      // cc-turn
      Configuration cc_goal_config =
          get_cc_goal_config(delta, hc_cc_circle_param);

      // regular cc-turn
      vector<Control> reg_cc_controls;
      cc_turn_controls(reg_hc_cc_circle, cc_goal_config, true, reg_cc_controls);
      vector<State> reg_cc_path =
          hc_cc_ss.integrate(start_state, reg_cc_controls);
      double reg_cc_path_length =
          accumulate(reg_cc_controls.begin(), reg_cc_controls.end(), 0.0,
                     [](double sum, Control const& control) {
                       return sum + fabs(control.delta_s);
                     });
      EXPECT_LT(fabs(reg_cc_path_length - get_path_length(reg_cc_path)),
                EPS_DISTANCE);

      // irregular cc-turn
      vector<Control> irreg_cc_controls;
      cc_turn_controls(irreg_hc_cc_circle, cc_goal_config, true,
                       irreg_cc_controls);
      vector<State> irreg_cc_path =
          hc_cc_ss.integrate(start_state, irreg_cc_controls);
      double irreg_cc_path_length =
          accumulate(irreg_cc_controls.begin(), irreg_cc_controls.end(), 0.0,
                     [](double sum, Control const& control) {
                       return sum + fabs(control.delta_s);
                     });
      EXPECT_LT(fabs(irreg_cc_path_length - get_path_length(irreg_cc_path)),
                EPS_DISTANCE);

      // hc-turn
      Configuration hc_goal_config =
          get_hc_goal_config(delta, hc_cc_circle_param);

      // regular hc-turn
      vector<Control> reg_hc_controls;
      hc_turn_controls(reg_hc_cc_circle, hc_goal_config, true, reg_hc_controls);
      vector<State> reg_hc_path =
          hc_cc_ss.integrate(start_state, reg_hc_controls);
      double reg_hc_path_length =
          accumulate(reg_hc_controls.begin(), reg_hc_controls.end(), 0.0,
                     [](double sum, Control const& control) {
                       return sum + fabs(control.delta_s);
                     });
      EXPECT_LT(fabs(reg_hc_path_length - get_path_length(reg_hc_path)),
                EPS_DISTANCE);

      // irregular hc-turn
      vector<Control> irreg_hc_controls;
      hc_turn_controls(irreg_hc_cc_circle, hc_goal_config, true,
                       irreg_hc_controls);
      vector<State> irreg_hc_path =
          hc_cc_ss.integrate(start_state, irreg_hc_controls);
      double irreg_hc_path_length =
          accumulate(irreg_hc_controls.begin(), irreg_hc_controls.end(), 0.0,
                     [](double sum, Control const& control) {
                       return sum + fabs(control.delta_s);
                     });
      EXPECT_LT(fabs(irreg_hc_path_length - get_path_length(irreg_hc_path)),
                EPS_DISTANCE);
    }
  }
}
...
TEST(HC_CC_Circle, reachingGoal) {
  ...
  for (auto const& sigma : sigmas) {
    ...
    for (auto const& delta : deltas) {
      ...
      vector<State> irreg_cc_path =
          hc_cc_ss.integrate(start_state, irreg_cc_controls);
      EXPECT_LT(get_distance(cc_goal_state, irreg_cc_path.back()),
                EPS_DISTANCE);
      ...
      vector<State> irreg_hc_path =
          hc_cc_ss.integrate(start_state, irreg_hc_controls);
      EXPECT_LT(get_distance(hc_goal_state, irreg_hc_path.back()),
                EPS_DISTANCE);
    }
  }
}
...
TEST(HC_CC_Circle, maxSharpness) {
  for (auto const& sigma : sigmas) {
    ...
    for (auto const& delta : deltas) {
      ...
      for (auto const& cc_control : cc_controls)
