...
class Path {
 public:
  /** \brief Constructor */
  Path(Configuration const& _start, Configuration const& _end, double _kappa,
       double _sigma, double _length);
...
int const nb_cc_dubins_paths = 7;

class CC_Dubins_Path : public Path {
 public:
  /** \brief Constructor */
  CC_Dubins_Path(Configuration const& _start, Configuration const& _end,
                 cc_dubins::path_type _type, double _kappa, double _sigma,
                 Configuration* _qi1, Configuration* _qi2, Configuration* _qi3,
                 Configuration* _qi4, HC_CC_Circle* _cstart,
                 HC_CC_Circle* _cend, HC_CC_Circle* _ci1, HC_CC_Circle* _ci2,
                 double _length);
...
/** \brief hc-/cc-reeds-shepp path types: E (Empty), S (Straight), T (Turn), c
 * (Cusp) */
namespace hc_cc_rs {
enum path_type {
  E,
...
int const nb_hc_cc_rs_paths = 18;

class HC_CC_RS_Path : public Path {
 public:
  /** \brief Constructor */
  HC_CC_RS_Path(Configuration const& _start, Configuration const& _end,
                hc_cc_rs::path_type _type, double _kappa, double _sigma,
                Configuration* _qi1, Configuration* _qi2, Configuration* _qi3,
                Configuration* _qi4, HC_CC_Circle* _cstart, HC_CC_Circle* _cend,
...
bool state_equal(State const& state1, State const& state2);

/** \brief Reverses a control */
void reverse_control(Control& control);

/** \brief Subtracts control2 from control1 */
Control subtract_control(Control const& control1, Control const& control2);
...
void straight_controls(Configuration const& q1, Configuration const& q2,
                       std::vector<Control>& controls);

/** \brief Appends controls with a rs-turn */
void rs_turn_controls(HC_CC_Circle const& c, Configuration const& q, bool order,
                      std::vector<Control>& controls);

/** \brief Appends controls with a hc-turn */
void hc_turn_controls(HC_CC_Circle const& c, Configuration const& q, bool order,
                      std::vector<Control>& controls);

/** \brief Appends controls with an elementary path if one exists */
bool cc_elementary_controls(HC_CC_Circle const& c, Configuration const& q,
                            double delta, bool order,
                            std::vector<Control>& controls);

/** \brief Appends controls with a default cc-turn consisting of two clothoids
 * and a circular arc */
void cc_default_controls(HC_CC_Circle const& c, Configuration const& q,
                         double delta, bool order,
                         std::vector<Control>& controls);

/** \brief Appends controls with a cc-turn */
void cc_turn_controls(HC_CC_Circle const& c, Configuration const& q, bool order,
                      std::vector<Control>& controls);
