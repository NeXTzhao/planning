 ...
 * @return Vec_Path
 */
Vec_Path calc_frenet_paths(float c_speed, float c_d, float c_d_d, float c_d_dd,
                           float s0) {
  std::vector<FrenetPath> fp_list;
  for (float di = -1 * MAX_ROAD_WIDTH; di < MAX_ROAD_WIDTH; di += D_ROAD_W) {
    for (float Ti = MINT; Ti < MAXT; Ti += DT) {
      FrenetPath fp;
      // QuinticPolynomial(float xs_, float vxs_, float axs_, float xe_, float
      // vxe_, float axe_, float T) : xs(xs_), vxs(vxs_), axs(axs_), xe(xe_),
      // vxe(vxe_), axe(axe_), a0(xs_), a1(vxs_), a2(axs_ / 2.0)
...
int main() {
  ...
  std::vector<Poi_f> obstcles{{{20.0, 10.0}},
                              {{30.0, 6.0}},
                              {{30.0, 8.0}},
                              {{35.0, 8.0}},
                              {{50.0, 3.0}}};
  ...
  for (int i = 0; i < SIM_LOOP; i++) {
    FrenetPath final_path = frenet_optimal_planning(csp_obj, s0, c_speed, c_d,
                                                    c_d_d, c_d_dd, obstcles);
    ...
    if (std::pow((final_path.x[1] - r_x.back()), 2) +
            std::pow((final_path.y[1] - r_y.back()), 2) <=
        1.0) {
      break;
    }
  }
