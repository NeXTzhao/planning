...
struct Lane : public XmlNode {
  Lane(std::string road_id, double lanesection_s0, int id, bool level,
       std::string type);
...
template <> struct std::hash<odr::LaneKey> {
  std::size_t operator()(const odr::LaneKey &key) const {
    return ((std::hash<string>()(key.road_id) ^
             (std::hash<double>()(key.lanesection_s0) << 1)) >>
            1) ^
           (std::hash<int>()(key.lane_id) << 1);
  }
};

template <> struct std::equal_to<odr::LaneKey> {
  bool operator()(const odr::LaneKey &lhs, const odr::LaneKey &rhs) const {
    return (lhs.road_id == rhs.road_id) &&
           (lhs.lanesection_s0 == rhs.lanesection_s0) &&
           (lhs.lane_id == rhs.lane_id);
  }
};

template <> struct std::less<odr::LaneKey> {
  bool operator()(const odr::LaneKey &lhs, const odr::LaneKey &rhs) const {
