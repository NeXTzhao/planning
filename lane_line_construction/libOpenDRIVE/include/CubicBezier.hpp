...
template <typename T, std::size_t Dim> struct CubicBezier {
  CubicBezier() = default;
  ...
  static std::array<Vec<T, Dim>, 4>
  get_control_points(const std::array<Vec<T, Dim>, 4> coefficients) {
    /* a + b*x + c*x^2 +d*x^3 */
  ...
  static std::array<Vec<T, Dim>, 4>
  get_coefficients(const std::array<Vec<T, Dim>, 4> control_points) {
...
std::array<Vec<T, Dim>, 4> CubicBezier<T, Dim>::get_subcurve(T t_start,
  ...
  auto f_cubic_t123 =
      [](const T &t1, const T &t2, const T &t3,
         const std::array<Vec<T, Dim>, 4> &ctrl_pts) -> Vec<T, Dim> {
    Vec<T, Dim> out;
    for (std::size_t dim = 0; dim < Dim; dim++) {
      out[dim] =
          (1 - t3) *
...
std::set<T> CubicBezier<T, Dim>::approximate_linear(T eps) const {
  ...
  for (const std::array<T, 2> &seg_intrvl : seg_intervals) {
    ...
    for (const double &p_sub : approximate_linear_quad_bezier<T, Dim>(
