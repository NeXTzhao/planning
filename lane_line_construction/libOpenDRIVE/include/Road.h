...
struct Crossfall : public CubicSpline {
  enum Side { Side_Both, Side_Left, Side_Right };
...
struct RoadLink : public XmlNode {
  enum ContactPoint { ContactPoint_None, ContactPoint_Start, ContactPoint_End };

  enum Type { Type_None, Type_Road, Type_Junction };
...
public:
  ...
  Vec3D get_xyz(double s, double t, double h, Vec3D *e_s = nullptr,
                Vec3D *e_t = nullptr, Vec3D *e_h = nullptr) const;
  Vec3D get_surface_pt(double s, double t, Vec3D *vn = nullptr) const;

  Line3D get_lane_border_line(const Lane &lane, double s_start, double s_end,
                              double eps, bool outer = true) const;
  Line3D get_lane_border_line(const Lane &lane, double eps,
                              bool outer = true) const;

  Mesh3D get_lane_mesh(const Lane &lane, double s_start, double s_end,
                       double eps,
                       std::vector<uint32_t> *outline_indices = nullptr) const;
  Mesh3D get_lane_mesh(const Lane &lane, double eps,
                       std::vector<uint32_t> *outline_indices = nullptr) const;

  Mesh3D get_roadmark_mesh(const Lane &lane, const RoadMark &roadmark,
                           double eps) const;
  Mesh3D get_road_object_mesh(const RoadObject &road_object, double eps) const;

  std::set<double> approximate_lane_border_linear(const Lane &lane,
                                                  double s_start, double s_end,
                                                  double eps,
                                                  bool outer = true) const;
  std::set<double> approximate_lane_border_linear(const Lane &lane, double eps,
                                                  bool outer = true) const;
