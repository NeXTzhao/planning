...
namespace odr {
template <class C, class T, T C::*member> struct PtrCmp {
...
template <typename T, std::size_t Dim,
          typename std::enable_if_t<std::is_arithmetic<T>::value> * = nullptr>
void rdp(const std::vector<Vec<T, Dim>> &points, const T epsilon,
...
template <typename T, std::size_t Dim,
          typename std::enable_if_t<std::is_arithmetic<T>::value> * = nullptr>
std::vector<T>
approximate_linear_quad_bezier(const std::array<Vec<T, Dim>, 3> &ctrl_pts,
                               T eps) {
  Vec<T, Dim> param_c;
  for (std::size_t dim = 0; dim < Dim; dim++)
...
template <typename T>
inline std::vector<T>
get_triangle_strip_outline_indices(const std::size_t num_vertices) {
