...
OpenDriveMap::OpenDriveMap(const std::string &xodr_file,
  ...
  for (pugi::xml_node junction_node : odr_node.children("junction")) {
    ...
    for (pugi::xml_node connection_node :
      ...
      CHECK_AND_REPAIR(contact_point_str == "start" ||
                           contact_point_str == "end",
    ...
    if (config.with_road_objects) {
      ...
      for (pugi::xml_node object_node :
        ...
        for (pugi::xml_node repeat_node : object_node.children("repeat")) {
          ...
          CHECK_AND_REPAIR(std::isnan(road_object_repeat.s0) ||
                               road_object_repeat.s0 >= 0,
    ...
    for (const auto &id_road : this->id_to_road) {
      ...
      for (auto s_lanesec_iter = road.s_to_lanesection.begin();
        ...
        if (find_successor &&
          ...
          next_lanesec =
              &next_road_contact_lanesec; // take prev. road to find predecessor
          next_lanesecs_road = &next_road;
        } else {
