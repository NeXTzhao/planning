...
double Poly3::get_max(double s_start, double s_end) const {
  if (this->d != 0) {
    ...
    const double max_val1 =
        this->get(std::min(std::max(s_extr, s_start), s_end));
    const double max_val2 =
        this->get(std::min(std::max(-s_extr, s_start), s_end));
...
std::set<double> Poly3::approximate_linear(double eps, double s_start,
                                           double s_end) const {
  if (s_start == s_end)
    return {};
  ...
  if (d == 0 && c != 0) {
    ...
    const double d_p = -d * s_0 * s_0 * s_0 + d * s_1 * s_1 * s_1 -
                       3 * d * s_0 * s_1 * s_1 + 3 * d * s_0 * s_0 * s_1;
    const double c_p = 3 * d * s_0 * s_0 * s_0 + 3 * d * s_0 * s_1 * s_1 -
                       6 * d * s_0 * s_0 * s_1 + c * s_0 * s_0 + c * s_1 * s_1 -
                       2 * c * s_0 * s_1;
    const double b_p = -3 * d * s_0 * s_0 * s_0 + 3 * d * s_0 * s_0 * s_1 -
                       2 * c * s_0 * s_0 + 2 * c * s_0 * s_1 - b * s_0 +
                       b * s_1;
    const double a_p = d * s_0 * s_0 * s_0 + c * s_0 * s_0 + b * s_0 + a;

    const std::array<Vec1D, 4> coefficients = {{{a_p}, {b_p}, {c_p}, {d_p}}};
    const std::set<double> p_vals =
        CubicBezier1D(CubicBezier1D::get_control_points(coefficients))
            .approximate_linear(eps);
...
bool Poly3::isnan() const {
  return (std::isnan(this->a) || std::isnan(this->b) || std::isnan(this->c) ||
          std::isnan(this->d));
}
...
double CubicSpline::get_grad(double s, double default_val,
                             bool extend_start) const {
  const Poly3 &poly = this->get_poly(s, extend_start);
  if (poly.isnan())
    return default_val;
...
double CubicSpline::get_max(double s_start, double s_end) const {
  ...
  for (auto s_poly_iter = s_start_poly_iter; s_poly_iter != s_end_poly_iter;
       s_poly_iter++) {
    const double s_start_poly = std::max(s_poly_iter->first, s_start);
    const double s_end_poly =
        (std::next(s_poly_iter) == s_end_poly_iter)
            ? s_end
            : std::min(std::next(s_poly_iter)->first, s_end);
    max_poly_vals.push_back(
        s_poly_iter->second.get_max(s_start_poly, s_end_poly));
  }

  const auto max_iter =
      std::max_element(max_poly_vals.begin(), max_poly_vals.end());
  const double max_val = (max_iter == max_poly_vals.end()) ? 0 : *max_iter;
  return max_val;
}

std::set<double> CubicSpline::approximate_linear(double eps, double s_start,
                                                 double s_end) const {
  if ((s_start == s_end) || this->s0_to_poly.empty())
    return {};
  ...
  for (auto s_poly_iter = s_start_poly_iter; s_poly_iter != s_end_poly_iter;
    ...
    std::set<double> s_vals_poly =
        s_poly_iter->second.approximate_linear(eps, s_start_poly, s_end_poly);
    if (s_vals_poly.size() < 2) {
      std::string err_msg =
          std::string("expected at least two sample points, got ") +
          std::to_string(s_vals_poly.size()) + std::string(" for [") +
          std::to_string(s_start_poly) + ' ' + std::to_string(s_end_poly) + ']';
