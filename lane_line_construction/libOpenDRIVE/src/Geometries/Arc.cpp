...
namespace odr {
Arc::Arc(double s0, double x0, double y0, double hdg0, double length,
         double curvature)
    : RoadGeometry(s0, x0, y0, hdg0, length, GeometryType_Arc),
      curvature(curvature) {}
...
std::set<double> Arc::approximate_linear(double eps) const {
  ...
  const double s_step =
      0.01 / std::abs(this->curvature); // sample at approx. every 1°
