...
public:
  ...
  template <typename T> void operator()(T const *points, size_t len);
...
private:
  ...
  template <typename T>
  Node *linkedList(T const *points, size_t len, const bool clockwise);
...
template <typename N>
template <typename T>
typename Earcut<N>::Node *Earcut<N>::linkedList(T const *points, size_t len,
