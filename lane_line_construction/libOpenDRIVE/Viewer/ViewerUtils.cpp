...
RoadNetworkMesh get_road_network_mesh(const OpenDriveMap &odr_map, double eps) {
  ...
  for (const auto &id_road : odr_map.id_to_road) {
    ...
    road_objects_mesh.road_start_indices[road_objects_mesh.vertices.size()] =
        road.id;
    ...
    for (const auto &s_lanesec : road.s_to_lanesection) {
      ...
      roadmarks_mesh.lanesec_start_indices[roadmarks_mesh.vertices.size()] =
          lanesec.s0;
      for (const auto &id_lane : lanesec.id_to_lane) {
        ...
        const std::vector<RoadMark> roadmarks =
            lane.get_roadmarks(lanesec.s0, road.get_lanesection_end(lanesec));
        for (const RoadMark &roadmark : roadmarks) {
          roadmarks_idx_offset = roadmarks_mesh.vertices.size();
          roadmarks_mesh.roadmark_type_start_indices[roadmarks_idx_offset] =
              roadmark.type;
