...
public:
  ...
  GetPreviousUntilLaneStart(double distance) const;
  ...
  GetAllLandmarksInDistance(double distance,
                            bool stop_at_junction = false) const;

  /// Returns a list of landmarks from the current position to a certain
  /// distance Filters by specified type
  std::vector<SharedPtr<Landmark>>
  GetLandmarksOfTypeInDistance(double distance, std::string filter_type,
                               bool stop_at_junction = false) const;
