...
static auto MakeMap(const std::string &opendrive_contents) {
  ...
  std::ifstream file(opendrive_contents);
  auto filename = std::string{std::istreambuf_iterator<char>(file),
                              std::istreambuf_iterator<char>()};
  auto map = opendrive::OpenDriveParser::Load(filename);
  if (!map.has_value()) {
...
Map::Map(rpc::MapInfo description, std::string xodr_content)
    : _description(std::move(description)), _map(MakeMap(xodr_content)) {
  open_drive_file = xodr_content;
}

Map::Map(std::string name, std::string xodr_content)
    : Map(rpc::MapInfo{std::move(name), std::vector<geom::Transform>{}},
          xodr_content) {
  open_drive_file = xodr_content;
}

Map::Map(std::string name) : _map(MakeMap(name)) {}

Map::~Map() = default;

SharedPtr<Waypoint> Map::GetWaypoint(geom::Location const &location,
...
SharedPtr<Waypoint> Map::GetWaypointXODR(carla::road::RoadId road_id,
...
Map::TopologyList Map::GetTopology() const {
  ...
  auto get_or_make_waypoint = [&](auto const &waypoint) {
    auto it = waypoints.find(waypoint);
    if (it == waypoints.end()) {
      it = waypoints
               .emplace(waypoint, SharedPtr<Waypoint>(new Waypoint{
                                      shared_from_this(), waypoint}))
               .first;
    }
    return it->second;
  };
  ...
  for (auto const &pair : topology) {
    result.emplace_back(get_or_make_waypoint(pair.first),
...
std::vector<SharedPtr<Waypoint>> Map::GenerateWaypoints(double distance) const {
  ...
  auto const waypoints = _map.GenerateWaypoints(distance);
...
std::vector<road::element::LaneMarking>
Map::CalculateCrossedLanes(geom::Location const &origin,
                           geom::Location const &destination) const {
  return _map.CalculateCrossedLanes(origin, destination);
}

geom::GeoLocation const &Map::GetGeoReference() const {
...
SharedPtr<Junction> Map::GetJunction(Waypoint const &waypoint) const {
  road::Junction const *juncptr =
      GetMap().GetJunction(waypoint.GetJunctionId());
  auto junction =
      SharedPtr<Junction>(new Junction(shared_from_this(), juncptr));
  return junction;
}

std::vector<std::pair<SharedPtr<Waypoint>, SharedPtr<Waypoint>>>
Map::GetJunctionWaypoints(road::JuncId id,
  ...
  for (auto &waypoint_pair : junction_waypoints) {
    result.emplace_back(std::make_pair(
        SharedPtr<Waypoint>(
            new Waypoint(shared_from_this(), waypoint_pair.first)),
        SharedPtr<Waypoint>(
            new Waypoint(shared_from_this(), waypoint_pair.second))));
  }
  return result;
}
...
Map::GetAllLandmarksOfType(std::string type) const {
  std::vector<SharedPtr<Landmark>> result;
  auto signal_references = _map.GetAllSignalReferences();
  for (auto *signal_reference : signal_references) {
...
std::vector<SharedPtr<Landmark>>
Map::GetLandmarkGroup(Landmark const &landmark) const {
  std::vector<SharedPtr<Landmark>> result;
  auto &controllers = landmark._signal->GetSignal()->GetControllers();
  for (auto &controller_id : controllers) {
    auto const &controller = _map.GetControllers().at(controller_id);
