...
Waypoint::Waypoint(SharedPtr<const Map> parent,
                   road::element::Waypoint waypoint)
    : _parent(std::move(parent)), _waypoint(std::move(waypoint)),
...
Waypoint::GetNextUntilLaneEnd(double distance) const {
  std::vector<SharedPtr<Waypoint>> result;
  ...
  double road_length =
      _parent->GetMap().GetLane(_waypoint).GetRoad()->GetLength();
...
std::vector<SharedPtr<Waypoint>>
Waypoint::GetPreviousUntilLaneStart(double distance) const {
  std::vector<SharedPtr<Waypoint>> result;
...
SharedPtr<Waypoint> Waypoint::GetRight() const {
  auto right_lane_waypoint = _parent->GetMap().GetRight(_waypoint);
  if (right_lane_waypoint.has_value()) {
    return SharedPtr<Waypoint>(
        new Waypoint(_parent, std::move(*right_lane_waypoint)));
  }
  return nullptr;
}

SharedPtr<Waypoint> Waypoint::GetLeft() const {
  auto left_lane_waypoint = _parent->GetMap().GetLeft(_waypoint);
  if (left_lane_waypoint.has_value()) {
    return SharedPtr<Waypoint>(
        new Waypoint(_parent, std::move(*left_lane_waypoint)));
  }
  return nullptr;
}

boost::optional<road::element::LaneMarking>
Waypoint::GetRightLaneMarking() const {
...
boost::optional<road::element::LaneMarking>
Waypoint::GetLeftLaneMarking() const {
...
template <typename EnumT> static EnumT operator&(EnumT lhs, EnumT rhs) {
...
road::element::LaneMarking::LaneChange Waypoint::GetLaneChange() const {
  ...
  if (((_waypoint.lane_id > 0) ? _waypoint.lane_id - 1
                               : _waypoint.lane_id + 1) > 0) {
    // if road goes backward
  ...
  return (c_right & lane_change_type::Right) |
         (c_left & lane_change_type::Left);
}

std::vector<SharedPtr<Landmark>>
Waypoint::GetAllLandmarksInDistance(double distance,
                                    bool stop_at_junction) const {
  std::vector<SharedPtr<Landmark>> result;
  auto signals = _parent->GetMap().GetSignalsInDistance(_waypoint, distance,
                                                        stop_at_junction);
  std::unordered_set<const road::element::RoadInfoSignal *>
      added_signals; // check for repeated signals
  for (auto &signal_data : signals) {
    ...
    auto waypoint =
        SharedPtr<Waypoint>(new Waypoint(_parent, signal_data.waypoint));
    result.emplace_back(new Landmark(waypoint, _parent, signal_data.signal,
                                     signal_data.accumulated_s));
  }
  return result;
}

std::vector<SharedPtr<Landmark>>
Waypoint::GetLandmarksOfTypeInDistance(double distance, std::string filter_type,
                                       bool stop_at_junction) const {
  std::vector<SharedPtr<Landmark>> result;
  std::unordered_set<const road::element::RoadInfoSignal *>
      added_signals; // check for repeated signals
