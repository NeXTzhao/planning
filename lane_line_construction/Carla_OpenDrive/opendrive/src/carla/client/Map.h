...
class Map : public EnableSharedFromThis<Map>, private NonCopyable {
public:
  ...
  const std::string &GetName() const { return _description.name; }

  const road::Map &GetMap() const { return _map; }

  const std::string &GetOpenDrive() const { return open_drive_file; }
  ...
  SharedPtr<Waypoint> GetWaypoint(const geom::Location &location,
                                  ...
                                  int32_t lane_type = static_cast<uint32_t>(
                                      road::Lane::LaneType::Driving)) const;

  SharedPtr<Waypoint> GetWaypointXODR(carla::road::RoadId road_id,
  ...
  GetAllLandmarksOfType(std::string type) const;

  /// Returns all the landmarks in the same group including this one
  std::vector<SharedPtr<Landmark>>
  GetLandmarkGroup(const Landmark &landmark) const;
