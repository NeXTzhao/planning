...
template <typename Protocol, typename EndPointType> class EndPoint;

template <typename Protocol> class EndPoint<Protocol, FullyDefinedEndPoint> {
public:
...
template <typename Protocol> static inline auto make_endpoint(uint16_t port) {
