...
#define DEBUG_ASSERT_EQ(lhs, rhs)                                              \
  DEBUG_ONLY(EXPECT_EQ(lhs, rhs));                                             \
  DEBUG_ASSERT(lhs == rhs);
#define DEBUG_ASSERT_NE(lhs, rhs)                                              \
  DEBUG_ONLY(EXPECT_NE(lhs, rhs));                                             \
  DEBUG_ASSERT(lhs != rhs);
#else
...
#define LIBCARLA_ASSERT_THROW__(pred, msg)                                     \
  if (!(pred)) {                                                               \
    ::carla::throw_exception(std::runtime_error(msg));                         \
  }

#define DEVELOPMENT_ASSERT(pred)                                               \
  DEVELOPMENT_ONLY(LIBCARLA_ASSERT_THROW__(pred, #pred))
