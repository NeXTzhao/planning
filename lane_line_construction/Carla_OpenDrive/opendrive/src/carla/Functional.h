...
public:
  ...
  template <typename FuncT> static auto MakeRecursive(FuncT &&func) {
  ...
  template <typename... FuncTs> static auto MakeOverload(FuncTs &&... fs) {
...
private:
  ...
  struct Overload<T, Ts...> : T, Overload<Ts...> {
    Overload(T &&func, Ts &&... rest)
        : T(std::forward<T>(func)), Overload<Ts...>(std::forward<Ts>(rest)...) {
    }
  ...
  template <typename T> struct Recursive {
    ...
    template <typename... Ts> auto operator()(Ts &&... arguments) const {
