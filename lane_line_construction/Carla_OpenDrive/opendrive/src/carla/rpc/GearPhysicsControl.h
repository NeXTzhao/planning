...
public:
  ...
  GearPhysicsControl(float in_ratio, float in_down_ratio, float in_up_ratio)
      : ratio(in_ratio), down_ratio(in_down_ratio), up_ratio(in_up_ratio) {}
  ...
  bool operator!=(const GearPhysicsControl &rhs) const {
    return ratio != rhs.ratio || down_ratio != rhs.down_ratio ||
  ...
  GearPhysicsControl(const FGearPhysicsControl &Gear)
      : ratio(Gear.Ratio), down_ratio(Gear.DownRatio), up_ratio(Gear.UpRatio) {}
