...
public:
  ...
  void AsyncRun(size_t worker_threads) { _server.async_run(worker_threads); }
...
template <typename R, typename... Args> struct FunctionWrapper<R (*)(Args...)> {
  ...
  static auto WrapSyncCall(boost::asio::io_context &io, FuncT &&functor) {
    return [&io, functor = std::forward<FuncT>(functor)](Metadata metadata,
                                                         Args... args) -> R {
      auto task =
          std::packaged_task<R()>([functor = std::move(functor), args...]() {
            return functor(args...);
          });
  ...
  template <typename FuncT> static auto WrapAsyncCall(FuncT &&functor) {
    return [functor = std::forward<FuncT>(functor)](
...
inline Server::Server(Args &&... args) : _server(std::forward<Args>(args)...) {
