...
public:
  ...
  WheelPhysicsControl(float in_tire_friction, float in_damping_rate,
                      float in_max_steer_angle, float in_radius,
                      float in_max_brake_torque, float in_max_handbrake_torque,
                      float in_lat_stiff_max_load, float in_lat_stiff_value,
                      float in_long_stiff_value, geom::Vector3D in_position)
      : tire_friction(in_tire_friction), damping_rate(in_damping_rate),
        max_steer_angle(in_max_steer_angle), radius(in_radius),
        ...
        long_stiff_value(in_long_stiff_value), position(in_position) {}
  ...
  bool operator!=(const WheelPhysicsControl &rhs) const {
    return tire_friction != rhs.tire_friction ||
           damping_rate != rhs.damping_rate ||
           max_steer_angle != rhs.max_steer_angle || radius != rhs.radius ||
           ...
           long_stiff_value != rhs.long_stiff_value || position != rhs.position;
  }
  ...
  WheelPhysicsControl(const FWheelPhysicsControl &Wheel)
      : tire_friction(Wheel.TireFriction), damping_rate(Wheel.DampingRate),
        max_steer_angle(Wheel.MaxSteerAngle), radius(Wheel.Radius),
  ...
  MSGPACK_DEFINE_ARRAY(tire_friction, damping_rate, max_steer_angle, radius,
