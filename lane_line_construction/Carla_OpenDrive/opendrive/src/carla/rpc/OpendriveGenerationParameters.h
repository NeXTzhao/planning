...
struct OpendriveGenerationParameters {
  ...
  OpendriveGenerationParameters(double v_distance, double max_road_len,
                                ...
                                bool e_pedestrian)
      : vertex_distance(v_distance), max_road_length(max_road_len),
        wall_height(w_height), additional_width(a_width),
        smooth_junctions(smooth_junc), enable_mesh_visibility(e_visibility),
  ...
  MSGPACK_DEFINE_ARRAY(vertex_distance, max_road_length, wall_height,
                       additional_width, smooth_junctions,
