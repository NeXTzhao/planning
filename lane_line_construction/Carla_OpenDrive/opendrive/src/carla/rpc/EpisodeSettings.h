...
public:
  ...
  EpisodeSettings(bool synchronous_mode, bool no_rendering_mode,
                  double fixed_delta_seconds = 0.0, bool substepping = true,
                  double max_substep_delta_time = 0.01, int max_substeps = 10,
                  float max_culling_distance = 0.0f,
                  bool deterministic_ragdolls = true,
      ...
      : synchronous_mode(synchronous_mode),
        ...
        fixed_delta_seconds(fixed_delta_seconds > 0.0
                                ? fixed_delta_seconds
                                : boost::optional<double>{}),
        substepping(substepping),
        max_substep_delta_time(max_substep_delta_time),
        max_substeps(max_substeps), max_culling_distance(max_culling_distance),
  ...
  bool operator==(const EpisodeSettings &rhs) const {
    return (synchronous_mode == rhs.synchronous_mode) &&
  ...
  bool operator!=(const EpisodeSettings &rhs) const { return !(*this == rhs); }
  ...
  EpisodeSettings(const FEpisodeSettings &Settings)
      : EpisodeSettings(
            ...
            Settings.FixedDeltaSeconds.Get(0.0), Settings.bSubstepping,
            Settings.MaxSubstepDeltaTime, Settings.MaxSubsteps,
            Settings.MaxCullingDistance, Settings.bDeterministicRagdolls,
            Settings.TileStreamingDistance, Settings.ActorActiveDistance) {
