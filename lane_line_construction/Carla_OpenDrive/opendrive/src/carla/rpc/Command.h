...
class Command {
private:
  template <typename T> struct CommandBase {
...
public:
  struct SpawnActor : CommandBase<SpawnActor> {
    ...
    SpawnActor(ActorDescription description, const geom::Transform &transform)
        : description(std::move(description)), transform(transform) {}
    SpawnActor(ActorDescription description, const geom::Transform &transform,
               ActorId parent)
        : description(std::move(description)), transform(transform),
          parent(parent) {}
  ...
  struct ApplyWalkerState : CommandBase<ApplyWalkerState> {
    ...
    ApplyWalkerState(ActorId id, const geom::Transform &value,
                     const float speed)
  ...
  struct ApplyTargetAngularVelocity : CommandBase<ApplyTargetAngularVelocity> {
    ...
    ApplyTargetAngularVelocity(ActorId id, const geom::Vector3D &value)
        : actor(id), angular_velocity(value) {}
    ActorId actor;
  ...
  struct SetAutopilot : CommandBase<SetAutopilot> {
    ...
    SetAutopilot(ActorId id, bool value, uint16_t tm_port)
        : actor(id), enabled(value), tm_port(tm_port) {}
    ActorId actor;
    bool enabled;
    uint16_t tm_port;
    MSGPACK_DEFINE_ARRAY(actor, enabled);
  ...
  using CommandType = boost::variant<
      SpawnActor, DestroyActor, ApplyVehicleControl, ApplyWalkerControl,
      ApplyVehiclePhysicsControl, ApplyTransform, ApplyWalkerState,
      ApplyTargetVelocity, ApplyTargetAngularVelocity, ApplyImpulse, ApplyForce,
      ApplyAngularImpulse, ApplyTorque, SetSimulatePhysics, SetEnableGravity,
      SetAutopilot, ShowDebugTelemetry, SetVehicleLightState>;
