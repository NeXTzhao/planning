...
public:
  ...
  WalkerControl(const FWalkerControl &Control)
      : direction(Control.Direction.X, Control.Direction.Y,
                  Control.Direction.Z),
        speed(1e-2f * Control.Speed), jump(Control.Jump) {}
  ...
  bool operator==(const WalkerControl &rhs) const { return !(*this != rhs); }
