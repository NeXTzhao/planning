...
public:
  ...
  bool operator!=(const FloatColor &rhs) const { return !(*this == rhs); }

#ifdef LIBCARLA_INCLUDED_FROM_UE4

  FloatColor(const FColor &color)
      : FloatColor(color.R / 255.f, color.G / 255.f, color.B / 255.f,
                   color.A / 255.f) {}
  ...
  FColor ToFColor() const {
    return FColor{static_cast<uint8>(r * 255), static_cast<uint8>(g * 255),
