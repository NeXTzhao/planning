...
public:
  ...
  ActorAttribute(const FActorAttribute &Attribute)
      : id(FromFString(Attribute.Id)),
        ...
        value(FromFString(Attribute.Value)), is_modifiable(false) {}
  ...
  MSGPACK_DEFINE_ARRAY(id, type, value, recommended_values, is_modifiable,
                       restrict_to_recommended);
};
...
public:
  ...
  ActorAttributeValue(const ActorAttribute &attribute)
      : id(attribute.id), type(attribute.type), value(attribute.value) {}
