...
void ControllerParser::Parse(const pugi::xml_document &xml,
  ...
  for (pugi::xml_node controller_node = opendrive_node.child("controller");
    ...
    const std::string controller_name =
        controller_node.attribute("name").value();
    const uint32_t controller_sequence =
        controller_node.attribute("sequence").as_uint();

    log_debug("Controller: ", controller_id, controller_name,
              controller_sequence);
    ...
    for (pugi::xml_node control_node : controller_node.children("control")) {
      ...
      // The controller_type is included in the OpenDrive format but not used
      // yet const std::string controller_type =
      // control_node.attribute("type").value();
      signals.insert(signal_id);
    }

    map_builder.CreateController(controller_id, controller_name,
                                 controller_sequence, std::move(signals));
  }
}
