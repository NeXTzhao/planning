...
void RoadParser::Parse(const pugi::xml_document &xml,
  ...
  for (pugi::xml_node node_road : xml.child("OpenDRIVE").children("road")) {
    ...
    if (link) {
      if (link.child("predecessor")) {
        road.predecessor =
            link.child("predecessor").attribute("elementId").as_uint();
      }
      if (link.child("successor")) {
        road.successor =
            link.child("successor").attribute("elementId").as_uint();
      }
    }
    ...
    // section offsets
    for (pugi::xml_node node_offset :
         node_road.child("lanes").children("laneOffset")) {
    ...
    // lane sections
    for (pugi::xml_node node_section :
         node_road.child("lanes").children("laneSection")) {
      LaneSection section{0.0, {}};

      section.s = node_section.attribute("s").as_double();

      // left lanes
      for (pugi::xml_node node_lane :
           node_section.child("left").children("lane")) {
        ...
        if (link2) {
          if (link2.child("predecessor")) {
            lane.predecessor =
                link2.child("predecessor").attribute("id").as_int();
          }
      ...
      // center lane
      for (pugi::xml_node node_lane :
           node_section.child("center").children("lane")) {
        Lane lane{0, road::Lane::LaneType::None, false, 0, 0};
      ...
      // right lane
      for (pugi::xml_node node_lane :
           node_section.child("right").children("lane")) {
        Lane lane{0, road::Lane::LaneType::None, false, 0, 0};
  ...
  // map_builder calls
  for (auto const r : roads) {
    carla::road::Road *road = map_builder.AddRoad(
        r.id, r.name, r.length, r.junction_id, r.predecessor, r.successor);
    ...
    for (auto const s : r.sections) {
      carla::road::LaneSection *section =
          map_builder.AddRoadSection(road, i++, s.s);

      // lanes
      for (auto const l : s.lanes) {
        /*carla::road::Lane *lane = */ map_builder.AddRoadSectionLane(
            section, l.id, static_cast<uint32_t>(l.type), l.level,
