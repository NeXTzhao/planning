...
static void ParseLanes(road::RoadId road_id, double s,
  ...
  for (pugi::xml_node lane_node : parent_node.children("lane")) {
    ...
    if (width_count == 0 && lane->GetId() != 0) {
      ...
      std::cout << "WARNING: In road " << lane->GetRoad()->GetId() << " lane "
                << lane->GetId()
    ...
    for (pugi::xml_node lane_road_mark : lane_node.children("roadMark")) {
      pugi::xml_node road_mark_type;
      {
        ...
        map_builder.CreateRoadMark(lane, road_mark_id, s_offset + s, type,
                                   weight, color, material, width, lane_change,
                                   height, type_name, type_width);
      }

      for (pugi::xml_node road_mark_type_line_node :
           road_mark_type.children("line")) {

        const double length =
            road_mark_type_line_node.attribute("length").as_double();
        const double space =
            road_mark_type_line_node.attribute("space").as_double();
        const double t =
            road_mark_type_line_node.attribute("tOffset").as_double();
        const double s_offset =
            road_mark_type_line_node.attribute("sOffset").as_double();
        const std::string rule =
            road_mark_type_line_node.attribute("rule").value();
        const double width =
            road_mark_type_line_node.attribute("width").as_double();

        // Call map builder for LaneRoadMarkType LaneRoadMarkTypeLine
        map_builder.CreateRoadMarkTypeLine(lane, road_mark_id, length, space, t,
                                           s_offset + s, rule, width);
      }
      ++road_mark_id;
    }
    ...
    for (pugi::xml_node lane_material_node : lane_node.children("material")) {
      ...
      const double s_offset =
          lane_material_node.attribute("sOffset").as_double();
      const std::string surface =
          lane_material_node.attribute("surface").value();
      const double friction =
          lane_material_node.attribute("friction").as_double();
      const double roughness =
          lane_material_node.attribute("roughness").as_double();

      // Create map builder for Lane Material
      map_builder.CreateLaneMaterial(lane, s_offset + s, surface, friction,
                                     roughness);
    }

    // Lane Visibility
    for (pugi::xml_node lane_visibility_node :
         lane_node.children("visibility")) {
      const double s_offset =
          lane_visibility_node.attribute("sOffset").as_double();
      const double forward =
          lane_visibility_node.attribute("forward").as_double();
      ...
      map_builder.CreateLaneVisibility(lane, s_offset + s, forward, back, left,
                                       right);
    }
    ...
    for (pugi::xml_node lane_access_node : lane_node.children("access")) {
      ...
      const std::string restriction =
          lane_access_node.attribute("restriction").value();
...
void LaneParser::Parse(const pugi::xml_document &xml,
                       carla::road::MapBuilder &map_builder) {
