...
void ObjectParser::Parse(const pugi::xml_document &xml,
    ...
    if (node_objects) {
      ...
      for (pugi::xml_node node_object : node_objects.children("object")) {
        ...
        if (type == "crosswalk") {
          ...
          if (node_outline) {
            points.clear();
            for (pugi::xml_node node_corner :
                 node_outline.children("cornerLocal")) {
          ...
          // create the object
          map_builder.AddRoadObjectCrosswalk(
              road, node_object.attribute("name").as_string(),
              ...
              node_object.attribute("length").as_double(), points);

        } else if (name.substr(0, 6) == "Speed_" ||
                   name.substr(0, 6) == "speed_") {
          road::RoadId road_id = node_road.attribute("id").as_uint();
          road::Road *road = map_builder.GetRoad(road_id);
          // speed signal by roadrunner
          std::string speed_str = name.substr(6);
          double speed = std::stod(speed_str);
          map_builder.AddSignal(
              road, node_object.attribute("id").as_string(),
              node_object.attribute("s").as_double(),
              node_object.attribute("t").as_double(),
              node_object.attribute("name").as_string(), "no",
              node_object.attribute("orientation").as_string(),
              node_object.attribute("zOffset").as_double(), "OpenDRIVE", "274",
              speed_str, speed, "mph",
              node_object.attribute("height").as_double(),
              node_object.attribute("width").as_double(), speed_str,
              node_object.attribute("hdg").as_double(),
              node_object.attribute("pitch").as_double(),
              node_object.attribute("roll").as_double());
          ...
          map_builder.AddSignal(
              ...
              node_object.attribute("name").as_string(), "no",
              node_object.attribute("orientation").as_string(),
              node_object.attribute("zOffset").as_double(), "OpenDRIVE", "206",
              "", 0, "mph", node_object.attribute("height").as_double(),
              node_object.attribute("width").as_double(), "",
              node_object.attribute("hdg").as_double(),
              node_object.attribute("pitch").as_double(),
              node_object.attribute("roll").as_double());
