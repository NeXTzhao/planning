...
void GeometryParser::Parse(const pugi::xml_document &xml,
  ...
  for (auto const geo : geometry) {
    ...
    if (geo.type == "line") {
      map_builder.AddRoadGeometryLine(road, geo.s, geo.x, geo.y, geo.hdg,
                                      geo.length);
    } else if (geo.type == "arc") {
      map_builder.AddRoadGeometryArc(road, geo.s, geo.x, geo.y, geo.hdg,
                                     geo.length, geo.arc.curvature);
    } else if (geo.type == "spiral") {
      map_builder.AddRoadGeometrySpiral(road, geo.s, geo.x, geo.y, geo.hdg,
                                        geo.length, geo.spiral.curvStart,
                                        geo.spiral.curvEnd);
    } else if (geo.type == "poly3") {
      map_builder.AddRoadGeometryPoly3(road, geo.s, geo.x, geo.y, geo.hdg,
                                       geo.length, geo.poly3.a, geo.poly3.b,
                                       geo.poly3.c, geo.poly3.d);
    } else if (geo.type == "paramPoly3") {
      map_builder.AddRoadGeometryParamPoly3(
          road, geo.s, geo.x, geo.y, geo.hdg, geo.length, geo.param_poly3.aU,
          geo.param_poly3.bU, geo.param_poly3.cU, geo.param_poly3.dU,
          geo.param_poly3.aV, geo.param_poly3.bV, geo.param_poly3.cV,
          geo.param_poly3.dV, geo.param_poly3.p_range);
    }
  }
}
