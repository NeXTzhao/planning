...
void JunctionParser::Parse(const pugi::xml_document &xml,
  ...
  for (pugi::xml_node junction_node : open_drive_node.children("junction")) {
    ...
    for (pugi::xml_node connection_node :
      ...
      connection.incoming_road =
          connection_node.attribute("incomingRoad").as_uint();
      connection.connecting_road =
          connection_node.attribute("connectingRoad").as_uint();
    ...
    for (pugi::xml_node controller_node :
      ...
      const road::ContId controller_id =
          controller_node.attribute("id").value();
      // const std::string controller_name =
      // controller_node.attribute("name").value(); const uint32_t
      // controller_sequence = controller_node.attribute("sequence").as_uint();
  ...
  for (auto &junction : junctions) {
    ...
    for (auto &connection : junction.connections) {
      map_builder.AddConnection(junction.id, connection.id,
                                connection.incoming_road,
                                connection.connecting_road);
      for (auto &lane_link : connection.lane_links) {
        map_builder.AddLaneLink(junction.id, connection.id, lane_link.from,
                                lane_link.to);
      }
    }
    map_builder.AddJunctionController(junction.id,
                                      std::move(junction.controllers));
  }
}
} // namespace parser
} // namespace opendrive
