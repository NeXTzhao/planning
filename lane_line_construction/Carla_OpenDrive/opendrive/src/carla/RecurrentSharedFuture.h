...
template <typename T> class RecurrentSharedFuture {
public:
  ...
  template <typename T2> void SetValue(const T2 &value);
  ...
  /// as such.
  template <typename ExceptionT> void SetException(ExceptionT &&exception);
...
public:
  SharedException()
      : _exception(std::make_shared<std::runtime_error>(
            "uninitialized SharedException")) {}

  SharedException(std::shared_ptr<std::exception> e)
      : _exception(std::move(e)) {}

  const char *what() const noexcept override { return _exception->what(); }

  std::shared_ptr<std::exception> GetException() const { return _exception; }
...
boost::optional<T> RecurrentSharedFuture<T>::WaitFor(time_duration timeout) {
  ...
  r.should_wait = true;
  if (!_cv.wait_for(lock, timeout.to_chrono(),
...
void RecurrentSharedFuture<T>::SetException(ExceptionT &&e) {
  SetValue(SharedException(
      std::make_shared<ExceptionT>(std::forward<ExceptionT>(e))));
}
