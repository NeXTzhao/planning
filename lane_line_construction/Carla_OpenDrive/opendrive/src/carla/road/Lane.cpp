...
const LaneSection *Lane::GetLaneSection() const { return _lane_section; }
...
LaneId Lane::GetId() const { return _id; }

Lane::LaneType Lane::GetType() const { return _type; }

bool Lane::GetLevel() const { return _level; }
...
ComputeTotalLaneWidth(const T container, const double s, const LaneId lane_id) {
...
geom::Transform Lane::ComputeTransform(const double s) const {
  ...
  if (GetId() < 0) {
    ...
    const auto computed_width = ComputeTotalLaneWidth(side_lanes, s, GetId());
  ...
  geom::Rotation rot(geom::Math::ToDegrees(static_cast<float>(dp.pitch)),
...
std::pair<geom::Vector3D, geom::Vector3D>
Lane::GetCornerPositions(const double s, const float extra_width) const {
  const Road *road = GetRoad();
  DEBUG_ASSERT(road != nullptr);
  ...
  if (extra_width != 0.f && road->IsJunction() &&
      GetType() == Lane::LaneType::Driving) {
  ...
  if (GetType() == LaneType::Sidewalk) {
    // RoadRunner doesn't export it right now and as a workarround where 15.24
    // cm is the exact height that match with most of the RoadRunner sidewalks
