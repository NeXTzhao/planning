...
public:
  ...
  const std::unordered_map<RoadId, Road> &GetRoads() const { return _roads; }
  ...
  template <typename T>
  auto GetLaneInfo(const RoadId road_id, const SectionId section_id,
                   const LaneId lane_id, const double s) {
    return GetRoad(road_id)
        .GetLaneById(section_id, lane_id)
        .template GetInfo<T>(s);
  }

  size_t GetRoadCount() const { return _roads.size(); }
