...
public:
  ...
  struct Connection {
    ...
    Connection(ConId id, RoadId incoming_road, RoadId connecting_road)
        : id(id), incoming_road(incoming_road),
          connecting_road(connecting_road), lane_links() {}
  };

  Junction(const JuncId id, const std::string name)
      : _id(id), _name(name), _connections() {}

  JuncId GetId() const { return _id; }
  ...
  carla::geom::BoundingBox GetBoundingBox() const { return _bounding_box; }
