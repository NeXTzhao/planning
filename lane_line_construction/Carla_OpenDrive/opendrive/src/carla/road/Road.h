...
public:
  ...
  GetNearestPoint(const geom::Location &loc) const;
  ...
  const std::pair<const Lane *, double> GetNearestLane(
      const double s, const geom::Location &loc,
  ...
  template <typename T>
  std::vector<const T *> GetInfosInRange(const double min_s,
...
public:
  auto GetLaneSectionsAt(const double s) {
    ...
    return MakeListView(iterator::make_map_values_iterator(pair.first),
                        iterator::make_map_values_iterator(pair.second));
  }

  auto GetLaneSectionsAt(const double s) const {
    auto pair = GetLessEqualRange(_lane_sections, s);
    return MakeListView(iterator::make_map_values_const_iterator(pair.first),
