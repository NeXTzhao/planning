...
double LaneSection::GetDistance() const { return _s; }
...
Road *LaneSection::GetRoad() const { return _road; }

SectionId LaneSection::GetId() const { return _id; }
...
std::map<LaneId, Lane> &LaneSection::GetLanes() { return _lanes; }

const std::map<LaneId, Lane> &LaneSection::GetLanes() const { return _lanes; }

std::vector<Lane *> LaneSection::GetLanesOfType(Lane::LaneType lane_type) {
  std::vector<Lane *> drivable_lanes;
  for (auto &&lane : _lanes) {
    if ((static_cast<uint32_t>(lane.second.GetType()) &
         static_cast<uint32_t>(lane_type)) > 0) {
