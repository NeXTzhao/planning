...
public:
  Signal(RoadId road_id, SignId signal_id, double s, double t, std::string name,
         std::string dynamic, std::string orientation, double zOffset,
         std::string country, std::string type, std::string subtype,
         double value, std::string unit, double height, double width,
         std::string text, double hOffset, double pitch, double roll)
      : _road_id(road_id), _signal_id(signal_id), _s(s), _t(t), _name(name),
        _dynamic(dynamic), _orientation(orientation), _zOffset(zOffset),
        _country(country), _type(type), _subtype(subtype), _value(value),
        _unit(unit), _height(height), _width(width), _text(text),
        _hOffset(hOffset), _pitch(pitch), _roll(roll) {}

  RoadId GetRoadId() const { return _road_id; }

  const SignId &GetSignalId() const { return _signal_id; }
  ...
  double GetHOffset() const { return _hOffset; }
  ...
  const std::set<ContId> &GetControllers() const { return _controllers; }
