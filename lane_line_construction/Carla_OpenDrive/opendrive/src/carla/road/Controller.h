...
public:
  Controller(ContId id, std::string name, uint32_t sequence)
      : _id(id), _name(name), _sequence(sequence) {}

  const ContId &GetControllerId() const { return _id; }

  const std::string &GetName() const { return _name; }

  const uint32_t &GetSequence() const { return _sequence; }

  const std::set<SignId> &GetSignals() const { return _signals; }

  const std::set<JuncId> &GetJunctions() const { return _junctions; }
