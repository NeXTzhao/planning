...
static void ForEachDrivableLaneImpl(RoadId road_id,
  ...
  for (const auto &pair : lane_section.GetLanes()) {
    ...
    if ((static_cast<uint32_t>(lane.GetType()) &
         static_cast<uint32_t>(Lane::LaneType::Driving)) > 0) {
      std::forward<FuncT>(func)(
          Waypoint{road_id, lane_section.GetId(), lane.GetId(),
...
template <typename FuncT>
static void ForEachLaneImpl(RoadId road_id, const LaneSection &lane_section,
                            double distance, Lane::LaneType lane_type,
                            FuncT &&func) {
  for (const auto &pair : lane_section.GetLanes()) {
    const auto &lane = pair.second;
    if (lane.GetId() == 0) {
    ...
    if ((static_cast<int32_t>(lane.GetType()) &
         static_cast<int32_t>(lane_type)) > 0) {
      std::forward<FuncT>(func)(
          Waypoint{road_id, lane_section.GetId(), lane.GetId(),
                   distance < 0.0 ? GetDistanceAtStartOfLane(lane) : distance});
    }
...
static void ForEachDrivableLane(const Road &road, FuncT &&func) {
  for (const auto &lane_section : road.GetLaneSections()) {
    ForEachDrivableLaneImpl(road.GetId(), lane_section,
                            -1.0, // At start of the lane
                            std::forward<FuncT>(func));
  }
}

/// Return a waypoint for each lane of the specified type on each lane section
/// of @a road.
template <typename FuncT>
static void ForEachLane(const Road &road, Lane::LaneType lane_type,
                        FuncT &&func) {
  for (const auto &lane_section : road.GetLaneSections()) {
    ForEachLaneImpl(road.GetId(), lane_section,
                    -1.0, // At start of the lane
                    lane_type, std::forward<FuncT>(func));
  }
}

/// Return a waypoint for each drivable lane at @a distance on @a road.
template <typename FuncT>
static void ForEachDrivableLaneAt(const Road &road, double distance,
                                  FuncT &&func) {
  for (const auto &lane_section : road.GetLaneSectionsAt(distance)) {
    ForEachDrivableLaneImpl(road.GetId(), lane_section, distance,
                            std::forward<FuncT>(func));
  }
}
...
static bool IsLanePresent(const MapData &data, Waypoint waypoint) {
  const auto &section =
      data.GetRoad(waypoint.road_id).GetLaneSectionById(waypoint.section_id);
...
boost::optional<Waypoint>
Map::GetClosestWaypointOnRoad(const geom::Location &pos,
                              int32_t lane_type) const {
  std::vector<Rtree::TreeElement> query_result =
      _rtree.GetNearestNeighboursWithFilter(
          Rtree::BPoint(pos.x, pos.y, pos.z),
  ...
  auto distance_to_segment = geom::Math::DistanceSegmentToPoint(
      pos, geom::Vector3D(s1.get<0>(), s1.get<1>(), s1.get<2>()),
...
boost::optional<Waypoint> Map::GetWaypoint(const geom::Location &pos,
                                           int32_t lane_type) const {
...
boost::optional<Waypoint> Map::GetWaypoint(RoadId road_id, LaneId lane_id,
                                           float s) const {
...
Map::GetMarkRecord(const Waypoint waypoint) const {
  ...
  const auto inner_lane_id =
      waypoint.lane_id < 0 ? waypoint.lane_id + 1 : waypoint.lane_id - 1;

  const auto &inner_lane =
      current_lane.GetRoad()->GetLaneById(waypoint.section_id, inner_lane_id);
...
std::vector<Map::SignalSearchData>
Map::GetSignalsInDistance(Waypoint waypoint, double distance,
  ...
  const double remaining_lane_length =
      forward ? lane.GetLength() - relative_s : relative_s;
  ...
  if (distance <= remaining_lane_length) {
    ...
    for (auto *signal : signals) {
      ...
      if (distance_to_signal == 0) {
        result.emplace_back(
            SignalSearchData{signal, waypoint, distance_to_signal});
      } else {
  ...
  const double signed_remaining_length =
      forward ? remaining_lane_length : -remaining_lane_length;

  // result = road.GetInfosInRange<RoadInfoSignal>(waypoint.s, waypoint.s +
  // signed_remaining_length);
  auto signals = road.GetInfosInRange<RoadInfoSignal>(
  ...
  for (auto &successor : GetSuccessors(waypoint)) {
    ...
    auto &sucessor_lane =
        _data.GetRoad(successor.road_id)
...
Map::CalculateCrossedLanes(const geom::Location &origin,
...
std::vector<geom::Location> Map::GetAllCrosswalkZones() const {
  ...
  for (const auto &pair : _data.GetRoads()) {
    ...
    std::vector<const RoadInfoCrosswalk *> crosswalks =
        road.GetInfos<RoadInfoCrosswalk>();
    if (crosswalks.size() > 0) {
      for (auto crosswalk : crosswalks) {
        ...
        pivot.rotation.yaw -= geom::Math::ToDegrees<float>(
            static_cast<float>(crosswalk->GetHeading()));
        pivot.rotation.yaw -=
            90; // move perpendicular to 's' for the lateral offset
        ...
        for (auto corner : crosswalk->GetPoints()) {
          geom::Vector3D v2(static_cast<float>(corner.u),
...
std::vector<Waypoint> Map::GetNext(const Waypoint waypoint,
  ...
  for (const auto &successor : GetSuccessors(waypoint)) {
    DEBUG_ASSERT(successor.road_id != waypoint.road_id ||
                 successor.section_id != waypoint.section_id ||
                 successor.lane_id != waypoint.lane_id);
    result = ConcatVectors(
        result, GetNext(successor, distance - remaining_lane_length));
  }
  return result;
}

std::vector<Waypoint> Map::GetPrevious(const Waypoint waypoint,
                                       const double distance) const {
  RELEASE_ASSERT(distance > 0.0);
  const auto &lane = GetLane(waypoint);
  const bool forward = !(waypoint.lane_id <= 0);
  ...
  for (const auto &successor : GetPredecessors(waypoint)) {
    DEBUG_ASSERT(successor.road_id != waypoint.road_id ||
                 ...
                 successor.lane_id != waypoint.lane_id);
    result = ConcatVectors(
        result, GetPrevious(successor, distance - remaining_lane_length));
  }
  return result;
}
...
boost::optional<Waypoint> Map::GetRight(Waypoint waypoint) const {
  ...
  return IsLanePresent(_data, waypoint) ? waypoint
                                        : boost::optional<Waypoint>{};
}
  ...
  for (const auto &pair : _data.GetRoads()) {
    ...
    for (double s = EPSILON; s < (road.GetLength() - EPSILON); s += distance) {
      ForEachDrivableLaneAt(
          road, s, [&](auto &&waypoint) { result.emplace_back(waypoint); });
    }
  }
  return result;
}
...
std::vector<Waypoint>
Map::GenerateWaypointsOnRoadEntries(Lane::LaneType lane_type) const {
  std::vector<Waypoint> result;
  for (const auto &pair : _data.GetRoads()) {
    const auto &road = pair.second;
    ...
    for (const auto &lane_section : road.GetLaneSectionsAt(0.0)) {
      for (const auto &lane : lane_section.GetLanes()) {
        ...
        if (lane.first < 0 && static_cast<int32_t>(lane.second.GetType()) &
                                  static_cast<int32_t>(lane_type)) {
          result.emplace_back(Waypoint{road.GetId(), lane_section.GetId(),
                                       lane.second.GetId(), 0.0});
        }
      }
    }
...
std::vector<Waypoint>
Map::GenerateWaypointsInRoad(RoadId road_id, Lane::LaneType lane_type) const {
  std::vector<Waypoint> result;
...
std::vector<std::pair<Waypoint, Waypoint>>
Map::GetJunctionWaypoints(JuncId id, Lane::LaneType lane_type) const {
  std::vector<std::pair<Waypoint, Waypoint>> result;
...
Map::ComputeJunctionConflicts(JuncId id) const {
  ...
  const Junction *junction = GetJunction(id);
  std::unordered_map<road::RoadId, std::unordered_set<road::RoadId>> conflicts;
  ...
  auto min_corner =
      geom::Vector3D(bbox_pos.x - bbox_ext.x, bbox_pos.y - bbox_ext.y,
  ...
  for (size_t i = 0; i < segments.size(); ++i) {
    ...
    Segment2d seg1{
        {segment1.first.first.get<0>(), segment1.first.first.get<1>()},
    ...
    for (size_t j = i + 1; j < segments.size(); ++j) {
      ...
      Segment2d seg2{
          {segment2.first.first.get<0>(), segment2.first.first.get<1>()},
...
const Lane &Map::GetLane(Waypoint waypoint) const {
  return _data.GetRoad(waypoint.road_id)
      .GetLaneById(waypoint.section_id, waypoint.lane_id);
}
...
void Map::AddElementToRtree(std::vector<Rtree::TreeElement> &rtree_elements,
  ...
  Rtree::BPoint end =
      Rtree::BPoint(next_transform.location.x, next_transform.location.y,
                    next_transform.location.z);
  rtree_elements.emplace_back(
      std::make_pair(Rtree::BSegment(init, end),
...
void Map::CreateRtree() {
  ...
  for (auto &waypoint : topology) {
    ...
    if (lane.IsStraight()) {
      ...
      double remaining_length = GetRemainingLength(lane, current_waypoint.s);
      ...
      AddElementToRtreeAndUpdateTransforms(rtree_elements, current_transform,
                                           current_waypoint, next_waypoint);
      // end of lane
    } else {
      ...
      while (true) {
        double delta_s = min_delta_s;
        double remaining_length = GetRemainingLength(lane, next_waypoint.s);
        remaining_length -= epsilon;
        delta_s = std::min(delta_s, remaining_length);

        if (delta_s < epsilon) {
          AddElementToRtreeAndUpdateTransforms(rtree_elements,
                                               current_transform,
                                               current_waypoint, next_waypoint);
          break;
        }
        ...
        geom::Transform next_transform = ComputeTransform(next_waypoint);
        double angle =
            geom::Math::GetVectorAngle(current_transform.GetForwardVector(),
                                       next_transform.GetForwardVector());

        if (std::abs(angle) > angle_threshold ||
            std::abs(current_waypoint.s - next_waypoint.s) >
                max_segment_length) {
          AddElementToRtree(rtree_elements, current_transform, next_transform,
                            current_waypoint, next_waypoint);
          current_waypoint = next_waypoint;
...
Junction *Map::GetJunction(JuncId id) { return _data.GetJunction(id); }

const Junction *Map::GetJunction(JuncId id) const {
  return _data.GetJunction(id);
}

geom::Mesh Map::GenerateMesh(const double distance, const float extra_width,
...
std::vector<std::unique_ptr<geom::Mesh>> Map::GenerateChunkedMesh(
  ...
  for (auto &&pair : _data.GetRoads()) {
    ...
    if (!road.IsJunction()) {
      ...
      out_mesh_list.insert(out_mesh_list.end(),
  ...
  for (const auto &junc_pair : _data.GetJunctions()) {
    ...
    if (params.smooth_junctions) {
      ...
      std::unique_ptr<geom::Mesh> junction_mesh =
  ...
  auto min_pos = geom::Vector2D(out_mesh_list.front()->GetVertices().front().x,
  ...
  size_t mesh_amount_x =
      static_cast<size_t>((max_pos.x - min_pos.x) / params.max_road_length) + 1;
  size_t mesh_amount_y =
      static_cast<size_t>((max_pos.y - min_pos.y) / params.max_road_length) + 1;
  ...
  for (auto &mesh : out_mesh_list) {
    auto vertex = mesh->GetVertices().front();
    size_t x_pos =
        static_cast<size_t>((vertex.x - min_pos.x) / params.max_road_length);
    size_t y_pos =
        static_cast<size_t>((vertex.y - min_pos.y) / params.max_road_length);
