...
public:
  ...
  Lane(LaneSection *lane_section, LaneId id,
      ...
      : _lane_section(lane_section), _id(id), _info(std::move(info)) {
  ...
  const std::vector<Lane *> &GetNextLanes() const { return _next_lanes; }

  const std::vector<Lane *> &GetPreviousLanes() const { return _prev_lanes; }

  LaneId GetSuccessor() const { return _successor; }

  LaneId GetPredecessor() const { return _predecessor; }
  ...
  GetCornerPositions(const double s, const float extra_width = 0.f) const;
