...
static std::vector<LaneMarking>
CrossingAtSameSection(const Map &map, const Waypoint *w0, const Waypoint *w1,
                      const bool w0_is_offroad, const bool dest_is_at_right) {
...
LaneCrossingCalculator::Calculate(const Map &map, const geom::Location &origin,
  ...
  geom::Vector3D dest_vec =
      (destination - origin)
          .MakeSafeUnitVector(2 * std::numeric_limits<float>::epsilon());

  // cross product
  const auto dest_is_at_right =
      (-orig_vec.x * dest_vec.y + orig_vec.y * dest_vec.x) < 0;

  return CrossingAtSameSection(map, &*w0, &*w1, w0_is_offroad,
                               dest_is_at_right);
}
