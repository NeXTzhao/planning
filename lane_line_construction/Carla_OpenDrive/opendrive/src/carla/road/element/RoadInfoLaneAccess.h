...
public:
  RoadInfoLaneAccess(double s, // start position relative to the position of the
                               // preceding lane section
                     std::string restriction)
      : RoadInfo(s), _restriction(restriction) {}

  void AcceptVisitor(RoadInfoVisitor &v) final { v.Visit(*this); }

  const std::string &GetRestriction() const { return _restriction; }
