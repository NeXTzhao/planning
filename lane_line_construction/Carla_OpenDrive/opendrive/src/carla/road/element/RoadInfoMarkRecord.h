...
public:
  ...
  RoadInfoMarkRecord(double s, int road_mark_id)
      : RoadInfo(s), _road_mark_id(road_mark_id), _type(""), _weight(""),
        _color("white"), _material("standard"), _width(0.15),
        _lane_change(LaneChange::None), _height(0.0), _type_name(""),
        _type_width(0.0) {}

  RoadInfoMarkRecord(double s, int road_mark_id, std::string type,
                     std::string weight, std::string color,
                     std::string material, double width, LaneChange lane_change,
                     double height, std::string type_name, double type_width)
      : RoadInfo(s), _road_mark_id(road_mark_id), _type(type), _weight(weight),
        _color(color), _material(material), _width(width),
        _lane_change(lane_change), _height(height), _type_name(type_name),
        _type_width(type_width) {}

  void AcceptVisitor(RoadInfoVisitor &v) final { v.Visit(*this); }

  /// Unique identifer for the road mark.
  int GetRoadMarkId() const { return _road_mark_id; }

  /// Type of the road mark.
  const std::string &GetType() const { return _type; }

  /// Weight of the road mark.
  const std::string &GetWeight() const { return _weight; }
  ...
  LaneChange GetLaneChange() const { return _lane_change; }
  ...
  const std::string &GetTypeName() const { return _type_name; }

  /// Width of the road mark type if it has one.
  double GetTypeWidth() const { return _type_width; }
