...
public:
  RoadInfoLaneVisibility(double s, // start position relative to the position of
                                   // the preceding lane section
                         double forward, double back, double left, double right)
      : RoadInfo(s), _forward(forward), _back(back), _left(left),
        _right(right) {}

  void AcceptVisitor(RoadInfoVisitor &v) final { v.Visit(*this); }

  double GetForward() const { return _forward; }

  double GetBack() const { return _back; }
