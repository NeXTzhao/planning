...
geom::Vector2D RotatebyAngle(double angle, double x, double y) {
  ...
  return geom::Vector2D(static_cast<float>(x * cos_a - y * sin_a),
...
GeometrySpiral::DistanceTo(const geom::Location &location) const {
...
DirectedPoint GeometryPoly3::PosFromDist(double dist) const {
  auto result =
      _rtree.GetNearestNeighbours(Rtree::BPoint(static_cast<float>(dist)))
          .front();
...
std::pair<float, float>
GeometryPoly3::DistanceTo(const geom::Location & /*p*/) const {
...
std::pair<float, float>
GeometryParamPoly3::DistanceTo(const geom::Location &) const {
  // No analytical expression (Newton-Raphson?/point search)
  // throw_exception(std::runtime_error("not implemented"));
  return {_start_position.x, _start_position.y};
}
...
void GeometryParamPoly3::PreComputeSpline() {
  ...
  double last_s = 0;
  RtreeValue last_val{last_u, last_v, last_s, _polyU.Tangent(param_p),
  ...
  for (size_t i = 0; i < number_intervals; ++i) {
    ...
    RtreeValue current_val{current_u, current_v, current_s, current_t_u,
                           current_t_v};
