...
const MapData *Road::GetMap() const { return _map_data; }

RoadId Road::GetId() const { return _id; }

std::string Road::GetName() const { return _name; }

double Road::GetLength() const { return _length; }

bool Road::IsJunction() const { return _is_junction; }

JuncId Road::GetJunctionId() const { return _junction_id; }

RoadId Road::GetSuccessor() const { return _successor; }

RoadId Road::GetPredecessor() const { return _predecessor; }

std::vector<Road *> Road::GetNexts() const { return _nexts; }

std::vector<Road *> Road::GetPrevs() const { return _prevs; }
...
element::DirectedPoint Road::GetDirectedPointIn(const double s) const {
  ...
  const auto lane_offset =
      _info.GetInfo<element::RoadInfoLaneOffset>(clamped_s);
  float offset = 0;
  if (lane_offset) {
    offset =
  ...
  element::DirectedPoint p =
      geometry->GetGeometry().PosFromDist(clamped_s - geometry->GetDistance());
...
Road::GetDirectedPointInNoLaneOffset(const double s) const {
  const auto clamped_s = geom::Math::Clamp(s, 0.0, _length);
  const auto geometry = _info.GetInfo<element::RoadInfoGeometry>(clamped_s);
...
Road::GetNearestPoint(const geom::Location &loc) const {
  ...
  for (auto g = geom_info_list.begin();
       g != geom_info_list.end() && g != nearest_geom; ++g) {
    DEBUG_ASSERT(*g != nullptr);
...
const std::pair<const Lane *, double>
Road::GetNearestLane(const double s, const geom::Location &loc,
  ...
  // positive left lanes
  auto left_lanes = MakeListView(lanes.lower_bound(1), lanes.end());
  ...
  for (const auto &lane : left_lanes) {
    const auto lane_width_info = lane.second->GetInfo<RoadInfoLaneWidth>(s);
    const auto half_width =
        -static_cast<float>(lane_width_info->GetPolynomial().Evaluate(s)) *
        0.5f;
