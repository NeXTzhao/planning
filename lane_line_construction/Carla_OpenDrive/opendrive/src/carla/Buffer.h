...
public:
  ...
  Buffer(Buffer &&rhs) noexcept
      : _parent_pool(std::move(rhs._parent_pool)), _size(rhs._size),
        _capacity(rhs._capacity), _data(rhs.pop()) {}
...
public:
  ...
  typename std::enable_if<
      !boost::asio::is_const_buffer_sequence<T>::value>::type
  copy_from(size_type offset, const T &source) {
  ...
  void copy_from(size_type offset, const TArray<T> &source) {
    copy_from(offset, reinterpret_cast<const value_type *>(source.GetData()),
              sizeof(T) * source.Num());
  }
#endif // LIBCARLA_INCLUDED_FROM_UE4
