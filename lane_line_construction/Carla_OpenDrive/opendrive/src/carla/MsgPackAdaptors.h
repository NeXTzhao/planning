...
MSGPACK_API_VERSION_NAMESPACE(MSGPACK_DEFAULT_API_NS) {
  ...
  template <typename T> struct convert<boost::optional<T>> {
    const clmdep_msgpack::object &operator()(const clmdep_msgpack::object &o,
  ...
  template <typename... Ts> struct convert<boost::variant<Ts...>> {
  ...
  private:
    template <uint64_t I>
    static void copy_to_variant_impl(const clmdep_msgpack::object &o,
                                     boost::variant<Ts...> &v) {
    ...
    static void
    copy_to_variant(const uint64_t index, const clmdep_msgpack::object &o,
                    boost::variant<Ts...> &v, std::index_sequence<Is...>) {
      std::initializer_list<int>(
          {(index == Is ? copy_to_variant_impl<Is>(o, v), 0 : 0)...});
    }
  };

  template <typename... Ts> struct pack<boost::variant<Ts...>> {
    template <typename Stream>
    packer<Stream> &operator()(clmdep_msgpack::packer<Stream> &o,
  ...
  template <typename... Ts> struct object_with_zone<boost::variant<Ts...>> {
    void operator()(clmdep_msgpack::object::with_zone &o,
                    const boost::variant<Ts...> &v) const {
      o.type = type::ARRAY;
      o.via.array.size = 2;
      o.via.array.ptr =
          static_cast<clmdep_msgpack::object *>(o.zone.allocate_align(
              ...
              MSGPACK_ZONE_ALIGNOF(clmdep_msgpack::object)));
      o.via.array.ptr[0] =
          clmdep_msgpack::object(static_cast<uint64_t>(v.which()), o.zone);
      boost::apply_visitor(
          [&](const auto &value) {
