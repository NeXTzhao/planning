...
public:
  ...
  Vector3D Abs() const { return Vector3D(abs(x), abs(y), abs(z)); }
  ...
  /// These 2 methods are explicitly deleted to avoid creating them by other
  /// users, unlike locations, some vectors have units and some don't, by
  /// removing these methods we found several places were the conversion from cm
  /// to m was missing
  ...
  Vector3D ToMeters() const { return *this * 1e-2f; }

  /// Return a Vector3D converted from meters to centimeters.
  Vector3D ToCentimeters() const { return *this * 1e2f; }
  ...
  void
  msgpack_object(MSGPACK_OBJECT *o,
                 clmdep_msgpack::zone &sneaky_variable_that_shadows_z) const {
    clmdep_msgpack::type::make_define_array(x, y, z).msgpack_object(
        o, sneaky_variable_that_shadows_z);
  }
  // =========================================================================
};
