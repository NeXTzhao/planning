...
struct MeshMaterial {
  ...
  MeshMaterial(const std::string &new_name, size_t start = 0u, size_t end = 0u)
      : name(new_name), index_start(start), index_end(end) {}
...
public:
  ...
  Mesh(const std::vector<vertex_type> &vertices = {},
      ...
      : _vertices(vertices), _normals(normals), _indexes(indexes), _uvs(uvs) {}
  ...
  operator FProceduralCustomMesh() const {
    ...
    for (const auto Vertex : GetVertices()) {
      ...
      Mesh.Vertices.Add(
          FVector{1e2f * Vertex.x, 1e2f * Vertex.y, 1e2f * Vertex.z});
    }
    ...
    for (const auto &Triangle : TriIndices) {
      ...
      if (Normal != FVector::ZeroVector) {
        // fix to prevent ugly x-fighting in geometries with very large
        // curvatures, ensures that all road geometry is facing upwards
