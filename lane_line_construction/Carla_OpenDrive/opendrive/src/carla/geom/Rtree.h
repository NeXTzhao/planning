...
template <typename T, size_t Dimension = 3> class PointCloudRtree {
public:
  typedef boost::geometry::model::point<float, Dimension,
  ...
  void InsertElement(const TreeElement &element) { _rtree.insert(element); }
  ...
  GetNearestNeighboursWithFilter(const BPoint &point, Filter filter,
                                 size_t number_neighbours = 1) const {
    std::vector<TreeElement> query_result;
    auto nearest = boost::geometry::index::nearest(
        point, static_cast<unsigned int>(number_neighbours));
    ...
    _rtree.query(operator&&(nearest, satisfies),
                 std::back_inserter(query_result));
    return query_result;
  }

  std::vector<TreeElement>
  GetNearestNeighbours(const BPoint &point,
                       size_t number_neighbours = 1) const {
    std::vector<TreeElement> query_result;
    _rtree.query(boost::geometry::index::nearest(
                     point, static_cast<unsigned int>(number_neighbours)),
                 std::back_inserter(query_result));
    return query_result;
  }
...
private:
  boost::geometry::index::rtree<TreeElement, boost::geometry::index::linear<16>>
      _rtree;
};
...
/// Useful to perform fast k-NN searches.
template <typename T, size_t Dimension = 3> class SegmentCloudRtree {
public:
  typedef boost::geometry::model::point<float, Dimension,
                                        boost::geometry::cs::cartesian>
      BPoint;
  ...
  void InsertElement(const BSegment &segment, const T &element_start,
                     const T &element_end) {
    _rtree.insert(
        std::make_pair(segment, std::make_pair(element_start, element_end)));
  }
  ...
  std::vector<TreeElement>
  GetNearestNeighboursWithFilter(const Geometry &geometry, Filter filter,
                                 size_t number_neighbours = 1) const {
    std::vector<TreeElement> query_result;
    _rtree.query(boost::geometry::index::nearest(
                     geometry, static_cast<unsigned int>(number_neighbours)) &&
  ...
  std::vector<TreeElement>
  GetNearestNeighbours(const Geometry &geometry,
                       size_t number_neighbours = 1) const {
    std::vector<TreeElement> query_result;
    _rtree.query(boost::geometry::index::nearest(
                     geometry, static_cast<unsigned int>(number_neighbours)),
  ...
  std::vector<TreeElement> GetIntersections(const Geometry &geometry) const {
    std::vector<TreeElement> query_result;
    _rtree.query(boost::geometry::index::intersects(geometry),
                 std::back_inserter(query_result));
    return query_result;
  }
