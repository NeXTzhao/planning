...
public:
  ...
  Vector3D GetForwardVector() const { return Math::GetForwardVector(*this); }

  Vector3D GetRightVector() const { return Math::GetRightVector(*this); }

  Vector3D GetUpVector() const { return Math::GetUpVector(*this); }
  ...
  void InverseRotateVector(Vector3D &in_point) const {
    ...
    Vector3D out_point;
    out_point.x =
        in_point.x * (cp * cy) + in_point.y * (cp * sy) + in_point.z * (sp);

    out_point.y = in_point.x * (cy * sp * sr - sy * cr) +
                  in_point.y * (sy * sp * sr + cy * cr) +
                  in_point.z * (-cp * sr);

    out_point.z = in_point.x * (-cy * sp * cr - sy * sr) +
  ...
  bool operator!=(const Rotation &rhs) const { return !(*this == rhs); }
