...
bool Mesh::IsValid() const {
  ...
  if (_vertices.empty()) {
    std::cout << "Mesh validation error: there are no vertices in the mesh."
              << std::endl;
    return false;
  }
  ...
  if (!_indexes.empty() && _indexes.size() % 3 != 0) {
    std::cout
        << "Mesh validation error: the index amount must be multiple of 3."
        << std::endl;
    return false;
  }

  if (!_materials.empty() && _materials.back().index_end == 0) {
    std::cout << "Mesh validation error: last material was not closed."
              << std::endl;
    return false;
  }
...
void Mesh::AddVertex(vertex_type vertex) { _vertices.push_back(vertex); }
...
void Mesh::AddNormal(normal_type normal) { _normals.push_back(normal); }

void Mesh::AddIndex(index_type index) { _indexes.push_back(index); }

void Mesh::AddUV(uv_type uv) { _uvs.push_back(uv); }
...
void Mesh::AddMaterial(const std::string &material_name) {
  ...
  if (!_materials.empty()) {
    if (_materials.back().index_end == 0) {
      ...
      // std::cout << "last material was not closed, closing it..." <<
      // std::endl;
      EndMaterial();
    }
  }
...
void Mesh::EndMaterial() {
  ...
  if (_materials.empty() || _materials.back().index_start == close_index ||
      _materials.back().index_end != 0) {
    // @todo: change this comment to a debug warning
    // std::cout << "WARNING: Bad end of material. Material not started." <<
    // std::endl;
    return;
  }
  if (_indexes.empty() || close_index % 3 != 0) {
    // @todo: change this comment to a debug warning
    // std::cout << "WARNING: Bad end of material. Face not started/ended." <<
    // std::endl;
    return;
  }
...
std::vector<Mesh::vertex_type> &Mesh::GetVertices() { return _vertices; }

size_t Mesh::GetVerticesNum() const { return _vertices.size(); }
...
size_t Mesh::GetIndexesNum() const { return _indexes.size(); }

const std::vector<Mesh::uv_type> &Mesh::GetUVs() const { return _uvs; }

const std::vector<Mesh::material_type> &Mesh::GetMaterials() const {
  return _materials;
}

size_t Mesh::GetLastVertexIndex() const { return _vertices.size(); }

Mesh &Mesh::operator+=(const Mesh &rhs) {
  const size_t v_num = GetVerticesNum();
  const size_t i_num = GetIndexesNum();

  _vertices.insert(_vertices.end(), rhs.GetVertices().begin(),
                   rhs.GetVertices().end());

  _normals.insert(_normals.end(), rhs.GetNormals().begin(),
                  rhs.GetNormals().end());

  std::transform(rhs.GetIndexes().begin(), rhs.GetIndexes().end(),
                 std::back_inserter(_indexes),
                 [=](size_t index) { return index + v_num; });

  _uvs.insert(_uvs.end(), rhs.GetUVs().begin(), rhs.GetUVs().end());

  std::transform(rhs.GetMaterials().begin(), rhs.GetMaterials().end(),
                 std::back_inserter(_materials), [=](MeshMaterial mat) {
