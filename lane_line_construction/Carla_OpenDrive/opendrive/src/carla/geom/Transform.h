...
namespace carla::geom {
    ...
    public:
        ...
        explicit Transform(const Location &in_location) : location(in_location), rotation() {}

        Transform(const Location &in_location, const Rotation &in_rotation)
                : location(in_location), rotation(in_rotation) {}

        // =========================================================================
        // -- Other methods --------------------------------------------------------
        // =========================================================================

        Vector3D GetForwardVector() const { return rotation.GetForwardVector(); }

        Vector3D GetRightVector() const { return rotation.GetRightVector(); }

        Vector3D GetUpVector() const { return rotation.GetUpVector(); }

        /// Applies this transformation to @a in_point (first translation then
        /// rotation).
        ...
        std::array<float, 16> GetMatrix() const {
            ...
            std::array<float, 16> transform = {cp * cy,
                                               cy * sp * sr - sy * cr,
                                               -cy * sp * cr - sy * sr,
                                               location.x,
                                               cp * sy,
        ...
        Transform(const FTransform &transform)
            : Transform(Location(transform.GetLocation()),
