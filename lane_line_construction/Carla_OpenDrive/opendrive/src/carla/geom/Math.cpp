...
std::pair<float, float> Math::DistanceSegmentToPoint(const Vector3D &p,
                                                     const Vector3D &v,
                                                     const Vector3D &w) {
...
std::pair<float, float> Math::DistanceArcToPoint(Vector3D p, Vector3D start_pos,
                                                 const float length,
  ...
  const Vector3D intersection =
      ((rotated_p - circ_center).MakeUnitVector() * radius) + circ_center;
  ...
  if (angle <= last_point_angle) {
    return std::make_pair(angle * radius, Distance2D(intersection, rotated_p));
  }
  ...
  const Vector3D end_pos(
      ...
      radius * std::sin(last_point_angle - pi_half) + circ_center.y, 0.0f);
  const float end_dist = Distance2D(end_pos, rotated_p);
  return (start_dist < end_dist) ? std::make_pair(0.0f, start_dist)
