...
public:
  ...
  explicit BoundingBox(const Location &in_location, const Vector3D &in_extent,
      ...
      : location(in_location), extent(in_extent), rotation(in_rotation) {}

  explicit BoundingBox(const Location &in_location, const Vector3D &in_extent)
      : location(in_location), extent(in_extent) {}

  explicit BoundingBox(const Vector3D &in_extent) : extent(in_extent) {}
   ...
   * @param in_world_point the point in world space that you want to query
   * whether it is inside or not.
   * @param in_bbox_to_world_transform the transformation from BoundingBox space
   * to World space.
   */
  bool Contains(const Location &in_world_point,
    ...
    return point_in_bbox_space.x >= -extent.x &&
           point_in_bbox_space.x <= extent.x &&
           point_in_bbox_space.y >= -extent.y &&
           point_in_bbox_space.y <= extent.y &&
           point_in_bbox_space.z >= -extent.z &&
           point_in_bbox_space.z <= extent.z;
  }

  /**
   *  Returns the positions of the 8 vertices of this BoundingBox in local
   * space.
   */
  std::array<Location, 8> GetLocalVertices() const {
    ...
    return {
        {location +
             Location(rotation.RotateVector({-extent.x, -extent.y, -extent.z})),
         location +
             Location(rotation.RotateVector({-extent.x, -extent.y, extent.z})),
         location +
             Location(rotation.RotateVector({-extent.x, extent.y, -extent.z})),
   ...
   * @param in_bbox_to_world_transform The Transform from this BoundingBox space
   * to world space.
   */
  std::array<Location, 8>
  GetWorldVertices(const Transform &in_bbox_to_world_tr) const {
    auto world_vertices = GetLocalVertices();
    std::for_each(world_vertices.begin(), world_vertices.end(),
                  [&in_bbox_to_world_tr](auto &world_vertex) {
  ...
  BoundingBox(const FBoundingBox &Box)
      : location(Box.Origin), extent(1e-2f * Box.Extent.X, 1e-2f * Box.Extent.Y,
                                     1e-2f * Box.Extent.Z),
