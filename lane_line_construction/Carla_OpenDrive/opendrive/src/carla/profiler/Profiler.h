...
public:
  explicit ProfilerData(std::string name, bool print_fps = false)
      : _name(std::move(name)), _print_fps(print_fps) {}
  ...
  float maximum() const { return ms(_max_elapsed); }

  float minimum() const { return ms(_min_elapsed); }
...
private:
  ...
  static inline float fps(float milliseconds) {
    return milliseconds > 0.0f ? (1e3f / milliseconds)
...
#define LIBCARLA_GTEST_GET_TEST_NAME()                                         \
...
#define CARLA_PROFILE_SCOPE(context, profiler_name)                            \
  static thread_local ::carla::profiler::detail::ProfilerData                  \
      carla_profiler_##context##_##profiler_name##_data(                       \
          LIBCARLA_GTEST_GET_TEST_NAME() + "." #context "." #profiler_name);   \
  ::carla::profiler::detail::ScopedProfiler                                    \
      carla_profiler_##context##_##profiler_name##_scoped_profiler(            \
          carla_profiler_##context##_##profiler_name##_data);
...
#define CARLA_PROFILE_FPS(context, profiler_name)                              \
  {                                                                            \
    ...
    if (!first_time) {                                                         \
      static thread_local ::carla::profiler::detail::ProfilerData              \
          profiler_data(LIBCARLA_GTEST_GET_TEST_NAME() + "." #context          \
                                                         "." #profiler_name,   \
                        true);                                                 \
