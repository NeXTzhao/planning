...
static void write_csv_to_stream(std::ostream &out, Arg &&arg, Args &&... args) {
  ...
  (void)expander{
      0,
      (void(out << ", " << std::setw(10) << std::forward<Args>(args)), 0)...};
}

class StaticProfiler {
public:
  StaticProfiler(std::string filename) : _filename(std::move(filename)) {
  ...
  template <typename... Args> void write_line(Args &&... args) {
    write_to_file(std::ios_base::app | std::ios_base::out,
...
ProfilerData::~ProfilerData() {
  ...
  if (_count > 0u) {
    if (_print_fps) {
      PROFILER.write_line(_name, fps(average()), fps(minimum()), fps(maximum()),
                          "FPS", _count);
    } else {
