...
static void test_road_elevation(const pugi::xml_document &xml,
  ...
  for (pugi::xml_node road_node : open_drive_node.children("road")) {
    ...
    for (pugi::xml_node elevation_profile_node : elevation_profile_nodes) {
      ...
      auto total_elevation_parser =
          std::distance(elevation_nodes.begin(), elevation_nodes.end());

      for (pugi::xml_node elevation_node : elevation_nodes) {
        float s = elevation_node.attribute("s").as_float();
        const auto elevation =
            map->GetMap().GetRoad(road_id).GetInfo<RoadInfoElevation>(s);
...
static void test_geometry(const pugi::xml_document &xml,
  ...
  for (pugi::xml_node road_node : open_drive_node.children("road")) {
    ...
    for (pugi::xml_node plan_view_nodes : road_node.children("planView")) {
      ...
      auto total_geometries_parser =
          std::distance(geometries_parser.begin(), geometries_parser.end());
      auto total_geometries = 0;
      for (pugi::xml_node geometry_node :
           plan_view_nodes.children("geometry")) {
        float s = geometry_node.attribute("s").as_float();
        auto geometry =
            map->GetMap().GetRoad(road_id).GetInfo<RoadInfoGeometry>(s);
        if (geometry != nullptr)
          ++total_geometries;
      }
...
static auto get_total_road_marks(
  ...
  for (pugi::xml_node lane_node : lane_nodes) {
    ...
    total_road_mark_parser +=
        std::distance(road_mark_nodes.begin(), road_mark_nodes.end());
    ...
    // <roadMark sOffset="0.0000000000000000e+0" type="none" material="standard"
    ...
    for (pugi::xml_node road_mark_node : road_mark_nodes) {
      ...
      const auto road_mark =
          lane->GetInfo<RoadInfoMarkRecord>(lane->GetDistance() + s_offset);
      if (road_mark != nullptr) {
        EXPECT_NEAR(lane->GetDistance() + s_offset, road_mark->GetDistance(),
                    error);
...
static void test_roads(const pugi::xml_document &xml,
  ...
  auto total_roads_parser =
      std::distance(roads_parser.begin(), roads_parser.end());
  ...
  for (pugi::xml_node road_node : roads_parser) {
    ...
    for (pugi::xml_node lanes_node : road_node.children("lanes")) {
      ...
      auto total_lane_sections_parser = std::distance(
          lane_sections_parser.begin(), lane_sections_parser.end());
      auto total_lane_sections =
          map->GetMap().GetRoad(road_id).GetLaneSections().size();
      ...
      for (pugi::xml_node lane_section_node : lane_sections_parser) {
        ...
        auto total_lanes_parser =
            std::distance(left_nodes.begin(), left_nodes.end());
        total_lanes_parser +=
            std::distance(right_nodes.begin(), right_nodes.end());
        total_lanes_parser +=
            std::distance(center_nodes.begin(), center_nodes.end());
        ...
        for (auto it = lane_section.begin(); it != lane_section.end(); ++it) {
          ...
          total_road_mark =
              total_left.first + total_center.first + total_right.first;
          total_road_mark_parser =
              total_left.first + total_center.first + total_right.first;
        }
...
static void test_junctions(const pugi::xml_document &xml,
  ...
  auto total_junctions_parser =
      std::distance(open_drive_node.children("junction").begin(),
                    open_drive_node.children("junction").end());
  ...
  for (pugi::xml_node junction_node : open_drive_node.children("junction")) {
    ...
    auto total_connections_parser =
        std::distance(junction_node.children("connection").begin(),
                      junction_node.children("connection").end());
    ...
    for (pugi::xml_node connection_node :
      ...
      auto total_lane_links_parser =
          std::distance(connection_node.children("laneLink").begin(),
                        connection_node.children("laneLink").end());
    ...
    results.push_back(pool.Post([file]() {
      ...
      for (auto i = 0u; i < number_of_waypoints_to_explore; ++i) {
        ...
        for (auto &&successor : map.GetSuccessors(wp)) {
          ASSERT_TRUE(successor.road_id != wp.road_id ||
                      successor.section_id != wp.section_id ||
                      successor.lane_id != wp.lane_id || successor.s != wp.s);
        }
        auto origin = wp;
        for (auto j = 0u; j < 200u; ++j) {
          ...
          for (auto k = 0u; k < number_of_next_wps_to_explore; ++k) {
            ...
            ++count;
            ASSERT_TRUE(next.road_id != wp.road_id ||
                        next.section_id != wp.section_id ||
                        next.lane_id != wp.lane_id || next.s != wp.s);
