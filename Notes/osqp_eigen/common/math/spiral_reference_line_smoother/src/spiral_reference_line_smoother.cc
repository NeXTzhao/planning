...
bool SpiralReferenceLineSmoother::Smooth(
  ...
  //                 config_.resolution());
  std::vector<common::PathPoint> smoothed_point2d = Interpolate(
      opt_theta, opt_kappa, opt_dkappa, opt_s, opt_x, opt_y, resolution);
...
int SpiralReferenceLineSmoother::SmoothStandAlone(
  ...
  ptop->set_default_max_point_deviation(spiral_max_deviation);
  ptop->set_element_weight_curve_length(spiral_weight_curve_length);
  ptop->set_element_weight_kappa(spiral_weight_kappa);
  ptop->set_element_weight_dkappa(spiral_weight_dkappa);
  ...
  // config_.spiral().max_iteration()); app->Options()->SetNumericValue("tol",
  // config_.spiral().opt_tol());
  // app->Options()->SetNumericValue("acceptable_tol",
  //                                 config_.spiral().opt_acceptable_tol());

  app->Options()->SetStringValue("hessian_approximation", "limited-memory");
  app->Options()->SetIntegerValue("max_iter", spiral_max_iteration);
  app->Options()->SetNumericValue("tol", spiral_opt_tol);
  app->Options()->SetNumericValue("acceptable_tol", spiral_opt_acceptable_tol);
...
bool SpiralReferenceLineSmoother::Smooth(std::vector<Eigen::Vector2d> point2d,
  ...
  app->Options()->SetIntegerValue("max_iter", spiral_max_iteration);
  app->Options()->SetIntegerValue("acceptable_iter",
                                  spiral_opt_acceptable_iteration);
  app->Options()->SetNumericValue("tol", spiral_opt_tol);
  app->Options()->SetNumericValue("acceptable_tol", spiral_opt_acceptable_tol);
  Ipopt::ApplicationReturnStatus status = app->Initialize();
  if (status != Ipopt::Solve_Succeeded) {
