...
std::pair<double, double> ReedShepp::calc_tau_omega(double const u,
                                                    double const v,
                                                    double const xi,
                                                    double const eta,
                                                    double const phi) {
...
bool ReedShepp::ShortestRSP(const std::shared_ptr<Node3d>& start_node,
  ...
  if (std::abs(all_possible_paths[optimal_path_index].x.back() -
    ...
    for (char segs_type : all_possible_paths[optimal_path_index].segs_types) {
      std::cout << "types are " << segs_type;
    }
...
bool ReedShepp::GenerateRSP(std::shared_ptr<Node3d> const& start_node,
...
bool ReedShepp::SCS(double const x, double const y, double const phi,
                    std::vector<ReedSheppPath>* all_possible_paths) {
  RSPParam SLS_param;
...
bool ReedShepp::CSC(double const x, double const y, double const phi,
                    std::vector<ReedSheppPath>* all_possible_paths) {
  RSPParam LSL1_param;
...
bool ReedShepp::CCC(double const x, double const y, double const phi,
                    std::vector<ReedSheppPath>* all_possible_paths) {
  RSPParam LRL1_param;
...
bool ReedShepp::CCCC(double const x, double const y, double const phi,
                     std::vector<ReedSheppPath>* all_possible_paths) {
...
bool ReedShepp::CCSC(double const x, double const y, double const phi,
                     std::vector<ReedSheppPath>* all_possible_paths) {
...
bool ReedShepp::CCSCC(double const x, double const y, double const phi,
                      std::vector<ReedSheppPath>* all_possible_paths) {
...
void ReedShepp::LSL(double const x, double const y, double const phi,
                    RSPParam* param) {
  // pair(r, theta)
...
void ReedShepp::LSR(double const x, double const y, double const phi,
                    RSPParam* param) {
  std::pair<double, double> polar =
      common::math::Cartesian2Polar(x + std::sin(phi), y - 1.0 - std::cos(phi));
...
void ReedShepp::LRL(double const x, double const y, double const phi,
                    RSPParam* param) {
  std::pair<double, double> polar =
      common::math::Cartesian2Polar(x - std::sin(phi), y - 1.0 + std::cos(phi));
...
void ReedShepp::SLS(double const x, double const y, double const phi,
                    RSPParam* param) {
...
void ReedShepp::LRLRn(double const x, double const y, double const phi,
                      RSPParam* param) {
...
void ReedShepp::LRLRp(double const x, double const y, double const phi,
                      RSPParam* param) {
  double xi = x + std::sin(phi);
  double eta = y - 1.0 - std::cos(phi);
...
void ReedShepp::LRSR(double const x, double const y, double const phi,
                     RSPParam* param) {
  double xi = x + std::sin(phi);
  double eta = y - 1.0 - std::cos(phi);
...
void ReedShepp::LRSL(double const x, double const y, double const phi,
                     RSPParam* param) {
  double xi = x - std::sin(phi);
  double eta = y - 1.0 + std::cos(phi);
...
void ReedShepp::LRSLR(double const x, double const y, double const phi,
                      RSPParam* param) {
  double xi = x + std::sin(phi);
  double eta = y - 1.0 - std::cos(phi);
  std::pair<double, double> polar = common::math::Cartesian2Polar(xi, eta);
...
bool ReedShepp::SetRSP(int const size, double const* lengths, char const* types,
                       std::vector<ReedSheppPath>* all_possible_paths) {
  ReedSheppPath path;
...
bool ReedShepp::GenerateLocalConfigurations(
  ...
  for (double & segs_length : shortest_path->segs_lengths) {
    segs_length =
        segs_length / max_kappa_;
  }
  shortest_path->total_length = shortest_path->total_length / max_kappa_;
  return true;
}

void ReedShepp::Interpolation(int const index, double const pd, char const m,
                              double const ox, double const oy,
                              double const ophi, std::vector<double>* px,
                              std ::vector<double>* py,
                              std::vector<double>* pphi,
                              std::vector<bool>* pgear) const {
  double ldx = 0.0;
  double ldy = 0.0;
  double gdx = 0.0;
  double gdy = 0.0;
