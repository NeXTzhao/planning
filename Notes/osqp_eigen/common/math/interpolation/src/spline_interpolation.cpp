...
interpolation::MultiSplineCoef generateSplineCoefficients(
  ...
  if (num_base > 2) {
    ...
    for (size_t i = 0; i < num_base - 2; ++i) {
      ...
      tdma_coef.d[i] = 6.0 * (diff_values[i + 1] / diff_keys[i + 1] -
  ...
  for (size_t i = 0; i < num_base - 1; ++i) {
    ...
    multi_spline_coef.c[i] = diff_values[i] / diff_keys[i] -
...
std::vector<double> slerp(const std::vector<double>& base_keys,
  ...
  const auto multi_spline_coef =
      generateSplineCoefficients(base_keys, base_values);
