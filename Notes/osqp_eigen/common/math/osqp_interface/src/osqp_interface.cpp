...
OSQPInterface::OSQPInterface(const Eigen::MatrixXd& P, const Eigen::MatrixXd& A,
...
void OSQPInterface::updateP(const Eigen::MatrixXd& P_new) {
  ...
  */
  ...
  osqp_update_P(m_work, P_csc.m_vals.data(), OSQP_NULL,
                static_cast<c_int>(P_csc.m_vals.size()));
}
...
void OSQPInterface::updateA(const Eigen::MatrixXd& A_new) {
  ...
  */
  ...
  osqp_update_A(m_work, A_csc.m_vals.data(), OSQP_NULL,
                static_cast<c_int>(A_csc.m_vals.size()));
  return;
}
...
void OSQPInterface::updateBounds(const std::vector<double>& l_new,
...
int64_t OSQPInterface::initializeProblem(const Eigen::MatrixXd& P,
  ...
  m_data->n = m_param_n;
  m_data->P = csc_matrix(
      m_data->n, m_data->n, static_cast<c_int>(P_csc.m_vals.size()),
      P_csc.m_vals.data(), P_csc.m_row_idxs.data(), P_csc.m_col_idxs.data());
  m_data->q = q_dyn;
  m_data->A = csc_matrix(
      m_data->m, m_data->n, static_cast<c_int>(A_csc.m_vals.size()),
      A_csc.m_vals.data(), A_csc.m_row_idxs.data(), A_csc.m_col_idxs.data());
  m_data->l = l_dyn;
  m_data->u = u_dyn;
...
std::tuple<std::vector<float64_t>, std::vector<float64_t>, int64_t, int64_t>
OSQPInterface::optimize(const Eigen::MatrixXd& P, const Eigen::MatrixXd& A,
                        const std::vector<float64_t>& q,
                        const std::vector<float64_t>& l,
                        const std::vector<float64_t>& u) {
